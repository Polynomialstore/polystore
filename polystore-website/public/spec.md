...
*   Retrieval session enforcement (Gamma‑4+): data-plane fetches MUST include `X‑PolyStore‑Session-Id` for **all served bytes**, and the server MUST reject out‑of‑session reads. Batching is preserved: a response MAY include multiple contiguous blobs as long as requests remain blob-aligned and a subset of the session’s range. After the owner ACK commits, a browser with a healthy user-gateway asks it to continue the exact session via `/gateway/retrieval/session-proof/continue`; the user-gateway derives the frozen payee from chain state. V2 sends `{session_id}` and MAY fall back to the frozen payee's public `/sp/retrieval/session-proof/continue` route. V3 sends only `{session_id,slot}` and the user-gateway authenticates its server-to-provider `/sp/session-proof` hop. Privileged proof submission endpoints remain provider-authenticated. The gateway is a relay/compute helper only; user authorization lives on‑chain (EVM precompile).
...
bytes; public slot/range limits still apply. Independent whole-list verification
reserves 1200000 gas per proof; PSB2 cross-session aggregation uses its measured
base-plus-marginal schedule.
