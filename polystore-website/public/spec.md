...
*   Retrieval session enforcement (Gamma‑4+): data-plane fetches MUST include `X‑PolyStore‑Session-Id` for **all served bytes**, and the server MUST reject out‑of‑session reads. Batching is preserved: a response MAY include multiple contiguous blobs as long as requests remain blob-aligned and a subset of the session’s range. After the owner ACK commits, the browser MAY ask the frozen payee to continue that exact session via `/sp/session-proof/continue`; this public request contains only `session_id`, while privileged proof submission and user-gateway relay endpoints remain provider-authenticated. The gateway is a relay/compute helper only; user authorization lives on‑chain (EVM precompile).
