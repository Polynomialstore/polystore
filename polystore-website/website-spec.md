# PolyStore Website Specification (v2.7)
...
|:---|:---|:---|
| `VITE_PUBLIC_DOMAIN` | *(empty)* | Optional domain override for auto endpoint inference (for example `polynomialstore.com` → `faucet/lcd/evm` subdomains). |
...
| `VITE_ENABLE_FAUCET` | auto (`1` on `*.polynomialstore.com`, else `0`) | Faucet UI/actions override (`1` force on, `0` force off). |
...
*   **Flow:**
    ...
    5.  Continue each owner-confirmed session or v3 obligation through the healthy user-gateway at `POST /gateway/retrieval/session-proof/continue`. Only v2 resolves the frozen payee and calls `POST /sp/retrieval/session-proof/continue` directly when the user-gateway is unavailable.
...
| **Storage Gateway (optional)** | `gatewayBase` | `http://127.0.0.1:8080` |
...
*   `POST /gateway/retrieval/session-proof/continue`: v2 `{session_id}` or v3 `{session_id,slot}` -> provider submission outcome. After the corresponding owner ACK commits, a healthy user-gateway derives the frozen payee from chain state and relays the request; it never accepts a provider routing hint or proof bytes.
...
*   **Actual (Gamma‑4):**
    ...
    5.  Client calls `POST /gateway/retrieval/session-proof/continue` after the owner ACK commits when the user-gateway is healthy. V2 may use direct frozen-payee `POST /sp/retrieval/session-proof/continue` as the unavailable-gateway fallback; v3 remains on the authenticated user-gateway relay.
...
```ts
	...
	const data = encodeFunctionData({ abi: POLYSTORE_PRECOMPILE_ABI, functionName: 'computeRetrievalSessionIds', args: [openRequests] })
	const result = await ethereum.request({ method: 'eth_call', params: [{ from, to: precompile, data }, 'latest'] })
	const [providers, sessionIds] = decodeFunctionResult({ abi: POLYSTORE_PRECOMPILE_ABI, functionName: 'computeRetrievalSessionIds', data: result })
```
