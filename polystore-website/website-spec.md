...
| `VITE_GATEWAY_BASE` | `http://127.0.0.1:8080` | Local gateway base URL. Non-loopback values are ignored; loopback values are normalized to `localhost` for consistent browser connectivity. |
...
| `VITE_ENABLE_FAUCET` | auto (`1` when `PUBLIC_DOMAIN !== ''`, else `0`) | Faucet UI/actions override (`1` force on, `0` force off). Auto-enable applies on any deployment where a public domain is inferred or resolved, including preview/public hosts, not only `*.polynomialstore.com`. |
