...
SWEEP_CONFIG_KEYS = [
    ...
    "slash_corrupt_retrieval_bps",
    "slash_invalid_synthetic_proof_bps",
...
def compute_signals(
    ...
    return {
        ...
        "economics": {
            ...
            "provider_bond_opportunity_cost": fnum(totals.get("provider_bond_opportunity_cost")),
...
def economic_assumption_lines(config: dict[str, Any]) -> list[str]:
    return [
        ...
        f"| Provider bond opportunity cost | `{fmt_bps(config.get('provider_bond_opportunity_cost_bps_per_epoch'))}` / epoch | Models the capital cost of locked collateral so bond sizing can be calibrated against provider churn pressure. |",
        ...
        f"| Hard-fault slash | `{fmt_money(config.get('slash_hard_fault'))}` fallback, `{fmt_bps(config.get('slash_hard_fault_bps'))}` generic bps, corrupt `{fmt_bps(config.get('slash_corrupt_retrieval_bps'))}`, invalid proof `{fmt_bps(config.get('slash_invalid_synthetic_proof_bps'))}` | Bps controls burn a share of remaining provider bond and override the legacy absolute fallback for calibrated slash sizing. |",
...
def diagnostic_signal_lines(signals: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"| Provider bond opportunity cost | `{fmt_money(economics['provider_bond_opportunity_cost'])}` | Shows the modeled carry cost of locked provider bond included in P&L and churn pressure. |",
...
def sweep_metric_meaning(key: str) -> str:
    meanings = {
        ...
        "provider_bond_opportunity_cost": "Modeled capital carry cost of locked provider bond included in provider P&L.",
