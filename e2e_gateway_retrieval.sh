...
#   - Verify that a Proof entry exists for that deal in /polystorechain/polystorechain/v1/proofs.
#
# Usage:
...
OWNER_ADDR="$("$ROOT_DIR/polystorechain/polystorechaind" keys show faucet -a --home "$CHAIN_HOME" --keyring-backend test 2>/dev/null || true)"
if [ -z "$OWNER_ADDR" ]; then
  echo "Failed to resolve faucet address from polystorechaind; check that the stack is running correctly."
  exit 1
fi
...
echo "Create-deal response: $CREATE_RESP"
	...
	DEAL_ID=""
	for i in {1..30}; do
	  DEALS_JSON="$(timeout 10s curl -s "$LCD_BASE/polystorechain/polystorechain/v1/deals")"
...
FOUND_PROOF="false"
for i in {1..30}; do
  PROOFS_JSON="$(timeout 10s curl -s "$LCD_BASE/polystorechain/polystorechain/v1/proofs")"
