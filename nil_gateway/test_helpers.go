...
func dynamicMockDealServer(dealStates map[uint64]struct {
	Owner string
	CID   string
}) *httptest.Server {
...
func requireOnchainSessionForTest(t *testing.T, required bool) {
	t.Helper()
	prev := requireOnchainSession
	requireOnchainSession = required
	t.Cleanup(func() { requireOnchainSession = prev })
}
