...
# Get Initial Heat
HEAT_INFO=$($NILCHAIND query polystorechain get-deal-heat --deal-id $DEAL_ID --output json 2>/dev/null || echo "{}")
...
sleep 10
HEAT_INFO_FINAL=$($NILCHAIND query polystorechain get-deal-heat --deal-id $DEAL_ID --output json)
