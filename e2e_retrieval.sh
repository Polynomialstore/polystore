...
POLYSTORE_CLI="./polystore_cli/target/release/polystore_cli"
CHAIN_ID="polystorechain"
HOME_DIR="./.polystorechain_retrieval"
...
for i in {1..12}
do
   yes | $BINARY tx polystorechain register-provider General 1000000000 --from "provider$i" --chain-id $CHAIN_ID --yes --home $HOME_DIR --keyring-backend test --broadcast-mode sync --node tcp://127.0.0.1:26657 > /dev/null
done
...
$POLYSTORE_CLI --trusted-setup $TRUSTED_SETUP shard $INPUT_FILE --out $SHARD_OUTPUT --save-mdu-prefix "$INPUT_FILE"
...
CREATE_RES=$(yes | $BINARY tx polystorechain create-deal 100 1000000 5000 --service-hint "General" --from user --chain-id $CHAIN_ID --yes --home $HOME_DIR --keyring-backend test --broadcast-mode sync --node tcp://127.0.0.1:26657 --output json)
...
yes | $BINARY tx polystorechain update-deal-content --deal-id $DEAL_ID --cid $MANIFEST_ROOT --size $SIZE --total-mdus 3 --witness-mdus 1 --from user --chain-id $CHAIN_ID --yes --home $HOME_DIR --keyring-backend test --broadcast-mode sync --node tcp://127.0.0.1:26657
...
$BINARY tx polystorechain sign-retrieval-receipt $DEAL_ID $PROVIDER1_ADDR 1 $MDU_FILE $TRUSTED_SETUP $MANIFEST_FILE 0 \
...
TX_RES=$(yes | $BINARY tx polystorechain submit-retrieval-proof $RECEIPT_FILE --from provider1 --chain-id $CHAIN_ID --yes --home $HOME_DIR --keyring-backend test --broadcast-mode sync --node tcp://127.0.0.1:26657 --output json)
echo $TX_RES
