# Policy Simulation Report: Viral Public Retrieval Spike

## Executive Summary

**Verdict:** `PASS`. This run simulates `viral-public-retrieval` with `96` providers, `500` data users, `48` deals, and an RS `8+4` layout for `6` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Expected policy behavior: High retrieval volume succeeds, provider payouts rise, base burns and sponsored-session spend are visible, owner escrow is not debited, and unnecessary repair stays quiet.
...
Use this to separate anti-wash controls from legitimate popularity handling and to plan sponsored-session keeper/accounting tests.
...
| Retrieval price/slot | `0.0150` |
| Sponsored retrieval share | `100.00%` |
| Owner retrieval debit share | `100.00%` |
...
| Retrieval price per slot | `0.0150` | Paid per successful provider slot served, before the configured variable burn. |
...
| Owner retrieval escrow debit | `100.00%` | Share of non-sponsored retrieval base and variable cost debited to owner escrow in scenarios that explicitly model owner-paid reads. |
...
| Wash accounted spend / net gain | `1125.0000` / `-99.0000` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
...
| Provider P&L p10 / p50 / p90 | `9.5470` / `9.9710` / `10.5540` | Shows whether aggregate P&L hides marginal-provider distress. |
...
| Retrieval price start/end/range | `0.0150` -> `0.0150` (`0.0150`-`0.0150`) | Shows whether demand pressure moved retrieval pricing. |
| Retrieval latent/effective attempts | `9000` / `9000` | Shows how much retrieval load was added by demand-shock multipliers. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 96 | 37.50% | 0 | 0 | 0 | 9.9975 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-070` | `global` | 6/16 | 37.50% | 0 | 848 | 0 | 0 | 11.2960 |
| `sp-062` | `global` | 6/16 | 37.50% | 0 | 845 | 0 | 0 | 11.2563 |
| `sp-064` | `global` | 6/16 | 37.50% | 0 | 823 | 0 | 0 | 10.9648 |
| `sp-036` | `global` | 6/16 | 37.50% | 0 | 812 | 0 | 0 | 10.8190 |
| `sp-066` | `global` | 6/16 | 37.50% | 0 | 811 | 0 | 0 | 10.8058 |
| `sp-067` | `global` | 6/16 | 37.50% | 0 | 808 | 0 | 0 | 10.7660 |
| `sp-045` | `global` | 6/16 | 37.50% | 0 | 799 | 0 | 0 | 10.6468 |
| `sp-042` | `global` | 6/16 | 37.50% | 0 | 797 | 0 | 0 | 10.6203 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 1.04% | 6 | 1.04% | 728 | 100.00% | 9.7060 |
| `op-001` | 1 | 1.04% | 6 | 1.04% | 758 | 100.00% | 10.1035 |
| `op-002` | 1 | 1.04% | 6 | 1.04% | 750 | 100.00% | 9.9975 |
| `op-003` | 1 | 1.04% | 6 | 1.04% | 729 | 100.00% | 9.7193 |
| `op-004` | 1 | 1.04% | 6 | 1.04% | 726 | 100.00% | 9.6795 |
| `op-005` | 1 | 1.04% | 6 | 1.04% | 716 | 100.00% | 9.5470 |
| `op-006` | 1 | 1.04% | 6 | 1.04% | 704 | 100.00% | 9.3880 |
| `op-007` | 1 | 1.04% | 6 | 1.04% | 726 | 100.00% | 9.6795 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 159.9600 | steady state |
| 2 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 159.9600 | steady state |
| 3 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 159.9600 | steady state |
| 4 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 159.9600 | steady state |
| 5 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 159.9600 | steady state |
| 6 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 159.9600 | steady state |
...
The run minted `75.1200` reward/audit units and burned `99.0000` units, for a burn-to-mint ratio of `131.79%`.

Providers earned `1095.1200` in modeled revenue against `135.3600` in modeled cost, ending with aggregate P&L `959.7600`.

Retrieval accounting paid providers `1026.0000`, burned `45.0000` in base fees, and burned `54.0000` in variable retrieval fees.

Wash-retrieval accounting shows explicit spend `1125.0000` against possible colluding-provider gain `-99.0000`.
...
Final modeled storage price was `1.0000` and retrieval price per slot was `0.0150`.

### Provider P&L Extremes

| Provider | Assigned Slots | Revenue | Cost | Slashed | P&L | Churn Risk |
|---|---:|---:|---:|---:|---:|---:|
| `sp-018` | 6 | 0.7200 + 9.9607 | 1.3590 | 0.0000 | 9.3217 | no |
| `sp-033` | 6 | 0.7200 + 9.9892 | 1.3610 | 0.0000 | 9.3482 | no |
| `sp-006` | 6 | 0.7200 + 10.0320 | 1.3640 | 0.0000 | 9.3880 | no |
| `sp-011` | 6 | 0.7200 + 10.1033 | 1.3690 | 0.0000 | 9.4543 | no |
| `sp-026` | 6 | 0.7200 + 10.1033 | 1.3690 | 0.0000 | 9.4543 | no |
...
| `min_retrieval_provider_payouts` | `PASS` | Legitimate high demand must pay providers for bandwidth. | retrieval_provider_payouts=1026, required>=100 |
| `max_data_loss_events` | `PASS` | Durability invariant: stress may allow unavailable reads, but modeled data loss must stay at zero. | data_loss_events=0, required<=0 |
| `min_retrieval_base_burned` | `PASS` | Requester/session demand must pay a non-zero base burn. | retrieval_base_burned=45, required>=1 |
| `min_sponsored_retrieval_attempts` | `PASS` | Sponsored public retrieval fixture must route demand through sponsor/requester-funded sessions. | sponsored_retrieval_attempts=9000, required>=1 |
| `min_sponsored_retrieval_spent` | `PASS` | Sponsored public retrieval fixture must pay non-zero session spend. | sponsored_retrieval_spent=1125, required>=1 |
| `exact_owner_retrieval_escrow_debited` | `PASS` | Sponsored public retrieval should keep owner deal escrow unchanged. | owner_retrieval_escrow_debited=0, required=0 |
