# Policy Simulation Report: Replacement Grinding

## Executive Summary

**Verdict:** `PASS`. This run simulates `replacement-grinding` with `36` providers, `80` data users, `18` deals, and an RS `8+4` layout for `10` epochs. Enforcement is configured as `REWARD_EXCLUSION`.

Model a repair path where pending replacement providers never prove readiness. The policy question is whether timeouts, cooldowns, and per-slot attempt caps bound churn instead of letting repairs remain pending forever.
...
| Repair attempt cap/slot | `3` (`0` means unlimited) |
| Repair backoff window | `2` epochs |
| Repair pending timeout | `1` epochs (`0` means disabled) |
...
The policy layer recorded `80` evidence events: `80` soft, `0` threshold, `0` hard, `0` economic, `0` market, and `0` spam events. Soft and economic evidence are suitable for repair and reward exclusion; hard or convicted threshold evidence is the category that can later justify slashing or stronger sanctions.

Repair was exercised: `18` repair operations started, `0` produced pending-provider readiness evidence, and `0` completed. The simulator models this as make-before-break reassignment, so the old assignment remains visible until replacement work catches up and the readiness gate is satisfied.

Reward exclusion was active: `0.8400` modeled reward units were burned instead of paid to non-compliant slots.

Repair coordination was constrained: `24` repair backoffs occurred across `18` repair attempts. Cooldown backoffs accounted for `18` events and attempt-cap backoffs accounted for `6` events. Pending-provider readiness timeouts accounted for `18` events.
...
| Repair readiness ratio | `0.00%` | Measures whether pending providers catch up before promotion. |
| Repair completion ratio | `0.00%` | Measures whether healing catches up with detection. |
| Repair attempts | `18` | Counts bounded attempts to open a repair or discover replacement pressure. |
| Repair backoff pressure | `1.3333` backoffs per started repair | Shows whether repair coordination is saturated. |
| Repair backoffs per attempt | `1.3333` | Distinguishes capacity/cooldown pressure from successful repair starts. |
| Repair cooldowns / attempt caps / readiness timeouts | `18` / `6` / `18` | Shows whether throttling, rather than candidate selection alone, is bounding repair churn. |
...
| Provider P&L p10 / p50 / p90 | `1.5365` / `1.6385` / `1.8000` | Shows whether aggregate P&L hides marginal-provider distress. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 36 | 37.50% | 106 | 0 | 1 | 1.5778 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-000` | `global` | 6/16 | 37.50% | 0 | 106 | 106 | 0 | -1.1000 |
| `sp-024` | `global` | 6/16 | 37.50% | 0 | 204 | 0 | 0 | 1.8340 |
| `sp-028` | `global` | 6/16 | 37.50% | 0 | 203 | 0 | 0 | 1.8255 |
| `sp-031` | `global` | 6/16 | 37.50% | 0 | 202 | 0 | 0 | 1.8170 |
| `sp-029` | `global` | 6/16 | 37.50% | 0 | 200 | 0 | 0 | 1.8000 |
| `sp-032` | `global` | 6/16 | 37.50% | 0 | 200 | 0 | 0 | 1.8000 |
| `sp-027` | `global` | 6/16 | 37.50% | 0 | 199 | 0 | 0 | 1.7915 |
| `sp-030` | `global` | 6/16 | 37.50% | 0 | 199 | 0 | 0 | 1.7915 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 2.77% | 6 | 2.77% | 106 | 0.00% | -1.1000 |
| `op-001` | 1 | 2.77% | 6 | 2.77% | 181 | 100.00% | 1.6385 |
| `op-002` | 1 | 2.77% | 6 | 2.77% | 165 | 100.00% | 1.5025 |
| `op-003` | 1 | 2.77% | 6 | 2.77% | 183 | 100.00% | 1.6555 |
| `op-004` | 1 | 2.77% | 6 | 2.77% | 192 | 100.00% | 1.7320 |
| `op-005` | 1 | 2.77% | 6 | 2.77% | 176 | 100.00% | 1.5960 |
| `op-006` | 1 | 2.77% | 6 | 2.77% | 172 | 100.00% | 1.5620 |
| `op-007` | 1 | 2.77% | 6 | 2.77% | 181 | 100.00% | 1.6385 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 38 | 6 | 0 | 0 | 0.1200 | 5.6800 | 26 offline responses, 6 quota misses, 6 delinquent slots |
| 2 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 5.6800 | 6 repair timeouts, 6 slots repairing, 6 delinquent slots |
| 3 | 100.00% | 26 | 0 | 0 | 0 | 0.1200 | 5.6800 | 14 offline responses, 6 quota misses, 6 repair backoffs, 6 repair cooldowns, 6 delinquent slots |
| 4 | 100.00% | 26 | 6 | 0 | 0 | 0.1200 | 5.6800 | 14 offline responses, 6 quota misses, 6 delinquent slots |
| 5 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 5.6800 | 6 repair timeouts, 6 slots repairing, 6 delinquent slots |
| 6 | 100.00% | 22 | 0 | 0 | 0 | 0.1200 | 5.6800 | 11 offline responses, 6 quota misses, 6 repair backoffs, 6 repair cooldowns, 6 delinquent slots |
| 7 | 100.00% | 26 | 6 | 0 | 0 | 0.1200 | 5.6800 | 15 offline responses, 6 quota misses, 6 delinquent slots |
| 8 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 5.6800 | 6 repair timeouts, 6 slots repairing, 6 delinquent slots |
| 9 | 100.00% | 20 | 0 | 0 | 0 | 0.1200 | 5.6800 | 10 offline responses, 6 quota misses, 6 repair backoffs, 6 repair cooldowns, 6 delinquent slots |
| 10 | 100.00% | 28 | 0 | 0 | 0 | 0.1200 | 5.6800 | 16 offline responses, 6 quota misses, 6 repair backoffs, 6 attempt caps, 6 delinquent slots |

## Enforcement Interpretation

The simulator recorded `80` evidence events and `60` repair ledger events. The first evidence epoch was `1` and the first repair-start epoch was `1`.

Evidence by reason:

- `quota_shortfall`: `42`
- `deputy_served_zero_direct`: `38`
...
- Repair attempts: `18`
- Repair backoffs: `24`
- Repair cooldown backoffs: `18`
- Repair attempt-cap backoffs: `6`
- Repair readiness timeouts: `18`
- Suspect slot-epochs: `0`
- Delinquent slot-epochs: `60`
- Final active slots in last epoch: `216`
...
|---:|---|---:|---:|---|---|---|---:|---:|
| 1 | `repair_started` | 1 | 0 | `sp-000` | `sp-019` | `deputy_served_zero_direct` | 1 | 0 |
| 1 | `repair_started` | 4 | 0 | `sp-000` | `sp-018` | `deputy_served_zero_direct` | 1 | 0 |
| 1 | `repair_started` | 7 | 0 | `sp-000` | `sp-022` | `deputy_served_zero_direct` | 1 | 0 |
| 1 | `repair_started` | 10 | 0 | `sp-000` | `sp-022` | `deputy_served_zero_direct` | 1 | 0 |
| 1 | `repair_started` | 13 | 0 | `sp-000` | `sp-017` | `deputy_served_zero_direct` | 1 | 0 |
| 1 | `repair_started` | 16 | 0 | `sp-000` | `sp-014` | `deputy_served_zero_direct` | 1 | 0 |
| 2 | `repair_timeout` | 1 | 0 | `sp-000` | `sp-019` | `readiness_timeout` | 1 | 4 |
| 2 | `repair_timeout` | 4 | 0 | `sp-000` | `sp-018` | `readiness_timeout` | 1 | 4 |
| 2 | `repair_timeout` | 7 | 0 | `sp-000` | `sp-022` | `readiness_timeout` | 1 | 4 |
| 2 | `repair_timeout` | 10 | 0 | `sp-000` | `sp-022` | `readiness_timeout` | 1 | 4 |
| 2 | `repair_timeout` | 13 | 0 | `sp-000` | `sp-017` | `readiness_timeout` | 1 | 4 |
| 2 | `repair_timeout` | 16 | 0 | `sp-000` | `sp-014` | `readiness_timeout` | 1 | 4 |
| ... | ... | ... | ... | ... | ... | `48` more events omitted | ... | ... |

## Economic Interpretation

The run minted `52.8400` reward/audit units and burned `4.8400` units, for a burn-to-mint ratio of `9.16%`.

Providers earned `102.8000` in modeled revenue against `46.0000` in modeled cost, ending with aggregate P&L `56.8000`.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-000` | 6 | 0.0000 + 0.0000 | 1.1000 | 0.0000 | -1.1000 | yes |
| `sp-020` | 6 | 1.2000 + 1.5390 | 1.2620 | 0.0000 | 1.4770 | no |
| `sp-002` | 6 | 1.2000 + 1.5675 | 1.2650 | 0.0000 | 1.5025 | no |
| `sp-013` | 6 | 1.2000 + 1.5960 | 1.2680 | 0.0000 | 1.5280 | no |
| `sp-021` | 6 | 1.2000 + 1.6055 | 1.2690 | 0.0000 | 1.5365 | no |
...
| `min_offline_responses` | `PASS` | Custom assertion. Review the detail and fixture threshold. | offline_responses=106, required>=1 |
| `min_repair_attempts` | `PASS` | Repair attempt accounting: constrained fixtures must visibly attempt repair before backing off. | repair_attempts=18, required>=1 |
| `min_repairs_started` | `PASS` | Repair liveness: policy must start reassignment when evidence warrants it. | repairs_started=18, required>=1 |
| `min_repair_timeouts` | `PASS` | Repair readiness timeout accounting: pending providers that fail catch-up must time out visibly. | repair_timeouts=18, required>=1 |
| `min_repair_cooldowns` | `PASS` | Repair cooldown accounting: repeated retry pressure must be throttled and visible. | repair_cooldowns=18, required>=1 |
| `min_repair_attempt_caps` | `PASS` | Repair attempt-cap accounting: bounded retry fixtures must hit and report the cap. | repair_attempt_caps=6, required>=1 |
| `max_repairs_completed` | `PASS` | Custom assertion. Review the detail and fixture threshold. | repairs_completed=0, required<=0 |
...
| 1 | 4 | 0 | `sp-000` | `soft` | `deputy_served_zero_direct` | `repair_candidate` |
| 1 | 4 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
| 1 | 7 | 0 | `sp-000` | `soft` | `deputy_served_zero_direct` | `repair_candidate` |
| 1 | 7 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
| 1 | 10 | 0 | `sp-000` | `soft` | `deputy_served_zero_direct` | `repair_candidate` |
| 1 | 10 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
| 1 | 13 | 0 | `sp-000` | `soft` | `deputy_served_zero_direct` | `repair_candidate` |
| 1 | 13 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
| 1 | 16 | 0 | `sp-000` | `soft` | `deputy_served_zero_direct` | `repair_candidate` |
| 1 | 16 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
