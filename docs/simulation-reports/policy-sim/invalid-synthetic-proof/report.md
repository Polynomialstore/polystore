# Policy Simulation Report: Invalid Synthetic Proof

## Executive Summary

**Verdict:** `PASS`. This run simulates `invalid-synthetic-proof` with `48` providers, `80` data users, `24` deals, and an RS `8+4` layout for `10` epochs. Enforcement is configured as `SLASH_SIMULATED`.
...
Expected policy behavior: Invalid proofs are recorded, repair starts and completes, the provider is simulated-slashed, corrupt retrieval payment remains zero, and durability remains intact.

Observed result: retrieval success was `100.00%`, reward coverage was `99.79%`, repairs started/ready/completed were `6` / `6` / `6`, and `1` providers ended with negative modeled P&L. The run recorded `0` unavailable reads, `0` expired retrieval rejections, `0` closed retrieval rejections, `0` modeled data-loss events, `0` bandwidth saturation responses and `0` repair backoffs across `6` repair attempts, with `0` pending-repair readiness timeouts. Slot health recorded `0` suspect slot-epochs and `12` delinquent slot-epochs. High-bandwidth promotions were `0` and final high-bandwidth providers were `0`.

## Review Focus

Use this fixture to isolate keeper proof-validation behavior from gateway byte-verification behavior before punitive hard-fault rollout.
...
| Provider P&L p10 / p50 / p90 | `1.1540` / `1.2305` / `1.3585` | Shows whether aggregate P&L hides marginal-provider distress. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 48 | 37.50% | 0 | 0 | 1 | 1.1008 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-038` | `global` | 7/16 | 43.75% | 0 | 173 | 0 | 0 | 1.6305 |
| `sp-037` | `global` | 7/16 | 43.75% | 0 | 164 | 0 | 0 | 1.5540 |
| `sp-025` | `global` | 7/16 | 43.75% | 0 | 163 | 0 | 0 | 1.5455 |
| `sp-035` | `global` | 7/16 | 43.75% | 0 | 153 | 0 | 0 | 1.4605 |
| `sp-042` | `global` | 6/16 | 37.50% | 0 | 149 | 0 | 0 | 1.3665 |
| `sp-040` | `global` | 6/16 | 37.50% | 0 | 148 | 0 | 0 | 1.3580 |
| `sp-036` | `global` | 6/16 | 37.50% | 0 | 147 | 0 | 0 | 1.3495 |
| `sp-039` | `global` | 6/16 | 37.50% | 0 | 147 | 0 | 0 | 1.3495 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-016` | 1 | 2.08% | 7 | 2.43% | 132 | 100.00% | 1.2820 |
| `op-021` | 1 | 2.08% | 7 | 2.43% | 141 | 100.00% | 1.3585 |
| `op-025` | 1 | 2.08% | 7 | 2.43% | 163 | 100.00% | 1.5455 |
| `op-035` | 1 | 2.08% | 7 | 2.43% | 153 | 100.00% | 1.4605 |
| `op-037` | 1 | 2.08% | 7 | 2.43% | 164 | 100.00% | 1.5540 |
| `op-038` | 1 | 2.08% | 7 | 2.43% | 173 | 100.00% | 1.6305 |
| `op-001` | 1 | 2.08% | 6 | 2.08% | 127 | 100.00% | 1.1795 |
| `op-002` | 1 | 2.08% | 6 | 2.08% | 133 | 100.00% | 1.2305 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 12 | 6 | 0 | 0 | 0.1200 | -0.2000 | 6 invalid proofs, 6 quota misses, 6 delinquent slots |
...
The simulator recorded `18` evidence events and `18` repair ledger events. The first evidence epoch was `1` and the first repair-start epoch was `1`.

Evidence by reason:

- `invalid_synthetic_proof`: `6`
- `slash:invalid_synthetic_proof`: `6`
- `quota_shortfall`: `6`
...
|---:|---|---:|---:|---|---|---|---:|---:|
| 1 | `repair_started` | 1 | 0 | `sp-000` | `sp-016` | `invalid_synthetic_proof` | 1 | 0 |
| 1 | `repair_started` | 5 | 0 | `sp-000` | `sp-025` | `invalid_synthetic_proof` | 1 | 0 |
| 1 | `repair_started` | 9 | 0 | `sp-000` | `sp-037` | `invalid_synthetic_proof` | 1 | 0 |
| 1 | `repair_started` | 13 | 0 | `sp-000` | `sp-021` | `invalid_synthetic_proof` | 1 | 0 |
| 1 | `repair_started` | 17 | 0 | `sp-000` | `sp-035` | `invalid_synthetic_proof` | 1 | 0 |
| 1 | `repair_started` | 21 | 0 | `sp-000` | `sp-038` | `invalid_synthetic_proof` | 1 | 0 |
| 3 | `repair_ready` | 1 | 0 | `sp-000` | `sp-016` | `catchup_ready` | 1 | 0 |
| 3 | `repair_completed` | 1 | 0 | `sp-000` | `sp-016` | `catchup_complete` | 1 | 0 |
| 3 | `repair_ready` | 5 | 0 | `sp-000` | `sp-025` | `catchup_ready` | 1 | 0 |
| 3 | `repair_completed` | 5 | 0 | `sp-000` | `sp-025` | `catchup_complete` | 1 | 0 |
| 3 | `repair_ready` | 9 | 0 | `sp-000` | `sp-037` | `catchup_ready` | 1 | 0 |
| 3 | `repair_completed` | 9 | 0 | `sp-000` | `sp-037` | `catchup_complete` | 1 | 0 |
...
Providers earned `118.0400` in modeled revenue against `59.2000` in modeled cost, ending with aggregate P&L `52.8400`.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-000` | 0 | 0.0000 + 0.0855 | 0.6290 | 6.0000 | -6.5435 | yes |
| `sp-008` | 6 | 1.2000 + 1.1685 | 1.2230 | 0.0000 | 1.1455 | no |
| `sp-010` | 6 | 1.2000 + 1.1685 | 1.2230 | 0.0000 | 1.1455 | no |
| `sp-029` | 6 | 1.2000 + 1.1685 | 1.2230 | 0.0000 | 1.1455 | no |
| `sp-004` | 6 | 1.2000 + 1.1780 | 1.2240 | 0.0000 | 1.1540 | no |
...
| `exact_corrupt_responses` | `PASS` | Invalid-proof-only fixture should not also exercise corrupt retrieval bytes. | corrupt_responses=0, required=0 |
...
|---:|---:|---:|---|---|---|---|
| 1 | 1 | 0 | `sp-000` | `hard` | `invalid_synthetic_proof` | `slash_simulated` |
| 1 |  |  | `sp-000` | `economic` | `slash:invalid_synthetic_proof` | `slash_simulated` |
| 1 | 1 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
| 1 | 5 | 0 | `sp-000` | `hard` | `invalid_synthetic_proof` | `slash_simulated` |
| 1 |  |  | `sp-000` | `economic` | `slash:invalid_synthetic_proof` | `slash_simulated` |
| 1 | 5 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
| 1 | 9 | 0 | `sp-000` | `hard` | `invalid_synthetic_proof` | `slash_simulated` |
| 1 |  |  | `sp-000` | `economic` | `slash:invalid_synthetic_proof` | `slash_simulated` |
| 1 | 9 | 0 | `sp-000` | `soft` | `quota_shortfall` | `repair_candidate` |
| 1 | 13 | 0 | `sp-000` | `hard` | `invalid_synthetic_proof` | `slash_simulated` |
| 1 |  |  | `sp-000` | `economic` | `slash:invalid_synthetic_proof` | `slash_simulated` |
