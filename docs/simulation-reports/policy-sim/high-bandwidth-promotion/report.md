# Policy Simulation Report: High-Bandwidth Provider Promotion

## Executive Summary

**Verdict:** `PASS`. This run simulates `high-bandwidth-promotion` with `96` providers, `300` data users, `48` deals, and an RS `8+4` layout for `8` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Use this before implementing provider capability state, hot-deal placement priority, or high-bandwidth reward multipliers in keeper/runtime code.
...
| High-bandwidth capacity threshold | `350` serves/epoch |
...
High-bandwidth capability policy was exercised: `55` providers were promoted, `0` were demoted, and hot retrievals received `22769` serves from high-bandwidth providers.
...
| High-bandwidth providers | `55` | Providers currently eligible for hot/high-bandwidth routing. |
| High-bandwidth promotions/demotions | `55` / `0` | Shows capability changes under measured demand. |
| Hot high-bandwidth serves/retrieval | `5.9186` | Measures whether hot retrievals actually use promoted providers. |
...
| Deals open/closed/expired | `48` / `0` / `0` | Confirms close/refund/expiry semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
| Provider P&L p10 / p50 / p90 | `1.8020` / `6.3899` / `6.9311` | Shows whether aggregate P&L hides marginal-provider distress. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 96 | 37.50% | 0 | 0 | 0 | 5.0000 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-075` | `global` | 6/16 | 37.50% | 419 | 575 | 0 | 0 | 7.1525 |
| `sp-049` | `global` | 6/16 | 37.50% | 416 | 574 | 0 | 0 | 7.1402 |
| `sp-080` | `global` | 6/16 | 37.50% | 356 | 570 | 0 | 0 | 7.0910 |
| `sp-082` | `global` | 6/16 | 37.50% | 461 | 569 | 0 | 0 | 7.0787 |
| `sp-056` | `global` | 6/16 | 37.50% | 451 | 565 | 0 | 0 | 7.0295 |
| `sp-050` | `global` | 6/16 | 37.50% | 455 | 561 | 0 | 0 | 6.9803 |
| `sp-077` | `global` | 6/16 | 37.50% | 383 | 560 | 0 | 0 | 6.9680 |
| `sp-078` | `global` | 6/16 | 37.50% | 375 | 560 | 0 | 0 | 6.9680 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 1.04% | 6 | 1.04% | 523 | 100.00% | 6.5129 |
| `op-001` | 1 | 1.04% | 6 | 1.04% | 516 | 100.00% | 6.4268 |
| `op-002` | 1 | 1.04% | 6 | 1.04% | 527 | 100.00% | 6.5621 |
| `op-003` | 1 | 1.04% | 6 | 1.04% | 130 | 100.00% | 1.6790 |
| `op-004` | 1 | 1.04% | 6 | 1.04% | 528 | 100.00% | 6.5744 |
| `op-005` | 1 | 1.04% | 6 | 1.04% | 521 | 100.00% | 6.4883 |
| `op-006` | 1 | 1.04% | 6 | 1.04% | 507 | 100.00% | 6.3161 |
| `op-007` | 1 | 1.04% | 6 | 1.04% | 530 | 100.00% | 6.5990 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
| 2 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
| 3 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
| 4 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
| 5 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
| 6 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
| 7 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
| 8 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 60.0000 | steady state |
...
The run minted `100.1600` reward/audit units and burned `41.2800` units, for a burn-to-mint ratio of `41.21%`.

Providers earned `602.8800` in modeled revenue against `122.8800` in modeled cost, ending with aggregate P&L `480.0000`.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-090` | 6 | 0.9600 + 1.3034 | 0.9780 | 0.0000 | 1.2854 | no |
| `sp-084` | 6 | 0.9600 + 1.3965 | 0.9850 | 0.0000 | 1.3715 | no |
| `sp-093` | 6 | 0.9600 + 1.4497 | 0.9890 | 0.0000 | 1.4207 | no |
| `sp-060` | 6 | 0.9600 + 1.5428 | 0.9960 | 0.0000 | 1.5068 | no |
| `sp-069` | 6 | 0.9600 + 1.5428 | 0.9960 | 0.0000 | 1.5068 | no |
...
| `min_high_bandwidth_promotions` | `PASS` | Capability promotion: measured fast providers should become high-bandwidth eligible. | high_bandwidth_promotions=55, required>=1 |
| `min_high_bandwidth_providers` | `PASS` | Final high-bandwidth provider count should be non-zero in promotion fixtures. | high_bandwidth_providers=55, required>=1 |
| `min_hot_retrieval_attempts` | `PASS` | Hot-service fixture must exercise hot retrieval demand. | hot_retrieval_attempts=3847, required>=1 |
| `min_hot_high_bandwidth_serves` | `PASS` | Hot-service routing must use promoted high-bandwidth providers. | hot_high_bandwidth_serves=22769, required>=1 |
| `max_high_bandwidth_demotions` | `PASS` | Promotion fixture should not immediately demote providers unless regression is modeled. | high_bandwidth_demotions=0, required<=0 |
