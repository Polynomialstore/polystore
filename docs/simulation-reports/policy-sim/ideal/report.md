# Policy Simulation Report: Cooperative Baseline

## Executive Summary

**Verdict:** `PASS`. This run simulates `ideal` with `48` providers, `80` data users, `24` deals, and an RS `8+4` layout for `8` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Expected policy behavior: No evidence, no repairs, no invalid proofs, full retrieval success, and full reward coverage.
...
Use this as the control case for policy deltas. If this report shows churn or missing rewards, the harness is overfitting enforcement noise.
...
| Provider P&L p10 / p50 / p90 | `0.8790` / `0.9640` / `1.1340` | Shows whether aggregate P&L hides marginal-provider distress. |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-030` | `global` | 6/16 | 37.50% | 0 | 133 | 0 | 0 | 1.2105 |
| `sp-028` | `global` | 6/16 | 37.50% | 0 | 129 | 0 | 0 | 1.1765 |
| `sp-031` | `global` | 6/16 | 37.50% | 0 | 129 | 0 | 0 | 1.1765 |
| `sp-026` | `global` | 6/16 | 37.50% | 0 | 127 | 0 | 0 | 1.1595 |
| `sp-035` | `global` | 6/16 | 37.50% | 0 | 125 | 0 | 0 | 1.1425 |
| `sp-025` | `global` | 6/16 | 37.50% | 0 | 124 | 0 | 0 | 1.1340 |
| `sp-033` | `global` | 6/16 | 37.50% | 0 | 124 | 0 | 0 | 1.1340 |
| `sp-034` | `global` | 6/16 | 37.50% | 0 | 122 | 0 | 0 | 1.1170 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 2.08% | 6 | 2.08% | 96 | 100.00% | 0.8960 |
| `op-001` | 1 | 2.08% | 6 | 2.08% | 98 | 100.00% | 0.9130 |
| `op-002` | 1 | 2.08% | 6 | 2.08% | 105 | 100.00% | 0.9725 |
| `op-003` | 1 | 2.08% | 6 | 2.08% | 100 | 100.00% | 0.9300 |
| `op-004` | 1 | 2.08% | 6 | 2.08% | 91 | 100.00% | 0.8535 |
| `op-005` | 1 | 2.08% | 6 | 2.08% | 90 | 100.00% | 0.8450 |
| `op-006` | 1 | 2.08% | 6 | 2.08% | 95 | 100.00% | 0.8875 |
| `op-007` | 1 | 2.08% | 6 | 2.08% | 100 | 100.00% | 0.9300 |
...
| 2 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 5.9200 | steady state |
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-005` | 6 | 0.9600 + 0.8550 | 0.9700 | 0.0000 | 0.8450 | no |
| `sp-004` | 6 | 0.9600 + 0.8645 | 0.9710 | 0.0000 | 0.8535 | no |
| `sp-013` | 6 | 0.9600 + 0.8645 | 0.9710 | 0.0000 | 0.8535 | no |
| `sp-015` | 6 | 0.9600 + 0.8835 | 0.9730 | 0.0000 | 0.8705 | no |
| `sp-020` | 6 | 0.9600 + 0.8835 | 0.9730 | 0.0000 | 0.8705 | no |
...
| `max_suspect_slots` | `PASS` | Healthy baseline should not produce suspect slot state. | suspect_slots=0, required<=0 |
| `max_delinquent_slots` | `PASS` | Transient jitter should not cross into delinquent slot state. | delinquent_slots=0, required<=0 |
| `max_quota_misses` | `PASS` | Healthy providers should not miss liveness quota. | quota_misses=0, required<=0 |
| `max_invalid_proofs` | `PASS` | Healthy providers should never produce invalid proofs. | invalid_proofs=0, required<=0 |
...
| `min_reward_coverage` | `PASS` | Healthy slots should receive the expected rewards. | reward_coverage=1, required>=1 |
