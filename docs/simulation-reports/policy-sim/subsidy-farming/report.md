...
| Owner-funded attempts / owner escrow debit | `1000` / `0.0000` | Detects whether public demand is unexpectedly draining the deal owner's escrow. |
| Wash accounted spend / net gain | `1.0000` / `75.0000` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
...
| Audit demand / spent | `0.4800` / `0.4800` | Shows whether enforcement evidence consumed the available audit budget. |
...
| Churn pressure provider-epochs / peak | `60` / `6` | Shows the breadth and duration of providers below the configured churn threshold. |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-025` | 1 | 1.38% | 12 | 2.08% | 136 | 100.00% | 4.7760 |
| `op-021` | 1 | 1.38% | 11 | 1.90% | 148 | 100.00% | 4.6480 |
| `op-067` | 1 | 1.38% | 11 | 1.90% | 140 | 100.00% | 4.5800 |
| `op-014` | 1 | 1.38% | 10 | 1.73% | 138 | 100.00% | 4.3330 |
| `op-022` | 1 | 1.38% | 10 | 1.73% | 144 | 100.00% | 4.3840 |
| `op-027` | 1 | 1.38% | 10 | 1.73% | 135 | 100.00% | 4.3075 |
| `op-028` | 1 | 1.38% | 10 | 1.73% | 124 | 100.00% | 4.2140 |
| `op-031` | 1 | 1.38% | 10 | 1.73% | 135 | 100.00% | 4.3075 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 48 | 0 | 0 | 0 | 2.4000 | 23.8400 | 48 quota misses, 48 suspect slots |
| 2 | 100.00% | 48 | 48 | 0 | 0 | 2.4000 | 23.8400 | 48 quota misses, 48 delinquent slots |
| 3 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 23.8400 | 48 slots repairing, 48 delinquent slots |
| 4 | 100.00% | 0 | 0 | 48 | 48 | 0.0000 | 23.8400 | 48 slots repairing |
| 5 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 26.2400 | steady state |
| 6 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 26.2400 | steady state |
| 7 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 26.2400 | steady state |
| 8 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 26.2400 | steady state |
| 9 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 26.2400 | steady state |
| 10 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 26.2400 | steady state |

## Enforcement Interpretation

The simulator recorded `96` evidence events and `144` repair ledger events. The first evidence epoch was `1` and the first repair-start epoch was `2`.
...
Wash-retrieval accounting shows explicit spend `1.0000` against possible colluding-provider gain `75.0000`.
...
Audit accounting saw `0.4800` of demand, spent `0.4800`, and ended with `0.0000` backlog after `0` exhausted epochs.
...
| `max_reward_coverage` | `PASS` | Custom assertion. Review the detail and fixture threshold. | reward_coverage=0.983050847, required<=0.995 |
