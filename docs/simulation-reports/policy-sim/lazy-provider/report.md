# Policy Simulation Report: Lazy Provider

## Executive Summary

**Verdict:** `PASS`. This run simulates `lazy-provider` with `48` providers, `80` data users, `24` deals, and an RS `8+4` layout for `10` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Reward exclusion was active: `0.2400` modeled reward units were burned instead of paid to non-compliant slots.
...
| Suspect / delinquent slot-epochs | `6` / `12` | Separates early warning state from threshold-crossed delinquency. |
...
| Provider P&L p10 / p50 / p90 | `1.0945` / `1.2475` / `1.3825` | Shows whether aggregate P&L hides marginal-provider distress. |
...
| Churn pressure provider-epochs / peak | `9` / `1` | Shows the breadth and duration of providers below the configured churn threshold. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 48 | 37.50% | 0 | 0 | 1 | 1.2233 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-025` | `global` | 7/16 | 43.75% | 0 | 166 | 0 | 0 | 1.5610 |
| `sp-038` | `global` | 7/16 | 43.75% | 0 | 155 | 0 | 0 | 1.4675 |
| `sp-030` | `global` | 6/16 | 37.50% | 0 | 151 | 0 | 0 | 1.3835 |
| `sp-041` | `global` | 7/16 | 43.75% | 0 | 150 | 0 | 0 | 1.4250 |
| `sp-029` | `global` | 6/16 | 37.50% | 0 | 149 | 0 | 0 | 1.3665 |
| `sp-033` | `global` | 6/16 | 37.50% | 0 | 149 | 0 | 0 | 1.3665 |
| `sp-031` | `global` | 6/16 | 37.50% | 0 | 148 | 0 | 0 | 1.3580 |
| `sp-026` | `global` | 6/16 | 37.50% | 0 | 147 | 0 | 0 | 1.3495 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-014` | 1 | 2.08% | 7 | 2.43% | 139 | 100.00% | 1.3315 |
| `op-022` | 1 | 2.08% | 7 | 2.43% | 145 | 100.00% | 1.3825 |
| `op-025` | 1 | 2.08% | 7 | 2.43% | 166 | 100.00% | 1.5610 |
| `op-038` | 1 | 2.08% | 7 | 2.43% | 155 | 100.00% | 1.4675 |
| `op-041` | 1 | 2.08% | 7 | 2.43% | 150 | 100.00% | 1.4250 |
| `op-045` | 1 | 2.08% | 7 | 2.43% | 147 | 100.00% | 1.3995 |
| `op-001` | 1 | 2.08% | 6 | 2.08% | 131 | 100.00% | 1.2135 |
...
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 6 | 0 | 0 | 0 | 0.1200 | 5.8000 | 6 quota misses, 6 suspect slots |
| 2 | 100.00% | 6 | 6 | 0 | 0 | 0.1200 | 5.8000 | 6 quota misses, 6 delinquent slots |
...
The simulator recorded `12` evidence events and `18` repair ledger events. The first evidence epoch was `1` and the first repair-start epoch was `2`.
...
|---:|---|---:|---:|---|---|---|---:|---:|
| 2 | `repair_started` | 1 | 0 | `sp-000` | `sp-022` | `quota_shortfall` | 1 | 0 |
| 2 | `repair_started` | 5 | 0 | `sp-000` | `sp-025` | `quota_shortfall` | 1 | 0 |
| 2 | `repair_started` | 9 | 0 | `sp-000` | `sp-041` | `quota_shortfall` | 1 | 0 |
| 2 | `repair_started` | 13 | 0 | `sp-000` | `sp-045` | `quota_shortfall` | 1 | 0 |
| 2 | `repair_started` | 17 | 0 | `sp-000` | `sp-014` | `quota_shortfall` | 1 | 0 |
| 2 | `repair_started` | 21 | 0 | `sp-000` | `sp-038` | `quota_shortfall` | 1 | 0 |
...
The run minted `67.3600` reward/audit units and burned `4.2400` units, for a burn-to-mint ratio of `6.29%`.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-000` | 0 | 0.0000 + 0.2375 | 0.7050 | 0.0000 | -0.4675 | yes |
| `sp-008` | 6 | 1.2000 + 1.0830 | 1.2140 | 0.0000 | 1.0690 | no |
| `sp-004` | 6 | 1.2000 + 1.1020 | 1.2160 | 0.0000 | 1.0860 | no |
| `sp-005` | 6 | 1.2000 + 1.1115 | 1.2170 | 0.0000 | 1.0945 | no |
...
|---|---|---|---|
| `min_quota_misses` | `PASS` | Fault fixture must generate quota evidence. | quota_misses=12, required>=1 |
...
| `min_suspect_slots` | `PASS` | Health-state observability: soft failures should become suspect before punitive consequences. | suspect_slots=6, required>=1 |
