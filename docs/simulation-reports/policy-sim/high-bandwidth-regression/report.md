# Policy Simulation Report: High-Bandwidth Capability Regression

## Executive Summary

**Verdict:** `PASS`. This run simulates `high-bandwidth-regression` with `72` providers, `200` data users, `36` deals, and an RS `8+4` layout for `8` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Expected policy behavior: Providers promote first, some promoted providers demote after sustained saturation, hot retrievals continue to succeed, and no data-loss event occurs.

Observed result: retrieval success was `100.00%`, reward coverage was `100.00%`, repairs started/ready/completed were `0` / `0` / `0`, and `0` providers ended with negative modeled P&L. The run recorded `0` unavailable reads, `0` expired retrieval rejections, `0` closed retrieval rejections, `0` modeled data-loss events, `21` bandwidth saturation responses and `0` repair backoffs across `0` repair attempts, with `0` pending-repair readiness timeouts. Slot health recorded `0` suspect slot-epochs and `0` delinquent slot-epochs. High-bandwidth promotions were `32` and final high-bandwidth providers were `30`.

## Review Focus

Use this before implementing capability demotion thresholds, hot-route failover behavior, or provider/operator alerting for regressed high-bandwidth service.
...
| Provider bandwidth range | `70`-`140` serves/epoch (`0` means unlimited) |
...
| High-bandwidth capacity threshold | `110` serves/epoch |
...
Provider bandwidth constraints mattered: the run recorded `21` saturated provider responses. That is a scale signal, not necessarily malicious behavior.

High-bandwidth capability policy was exercised: `32` providers were promoted, `2` were demoted, and hot retrievals received `21062` serves from high-bandwidth providers.
...
| Saturation rate | `0.44%` | Provider bandwidth saturation per retrieval attempt. |
| Peak saturation | `18` at epoch `2` | Reveals when bandwidth, not storage correctness, became the bottleneck. |
...
| High-bandwidth providers | `30` | Providers currently eligible for hot/high-bandwidth routing. |
| High-bandwidth promotions/demotions | `32` / `2` | Shows capability changes under measured demand. |
| Hot high-bandwidth serves/retrieval | `4.3879` | Measures whether hot retrievals actually use promoted providers. |
...
| Owner-funded attempts / owner escrow debit | `4800` / `0.0000` | Detects whether public demand is unexpectedly draining the deal owner's escrow. |
| Wash accounted spend / net gain | `14.4000` / `496.3200` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
...
| Deals open/closed/expired | `36` / `0` / `0` | Confirms close/refund/expiry semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
| Top operator provider share | `1.38%` | Shows whether many SP identities are controlled by one operator. |
| Top operator assignment share | `1.38%` | Shows whether placement caps translate identity concentration into slot concentration. |
...
| Provider P&L p10 / p50 / p90 | `2.5277` / `5.7872` / `9.9938` | Shows whether aggregate P&L hides marginal-provider distress. |
...
| Active / exited / reserve provider capacity | `1152` / `0` / `0` slots | Measures supply remaining, removed, and still waiting outside normal placement. |
...
| Retrieval latent/effective attempts | `4800` / `4800` | Shows how much retrieval load was added by demand-shock multipliers. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 72 | 37.50% | 0 | 21 | 0 | 6.6400 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-026` | `global` | 6/16 | 37.50% | 110 | 510 | 0 | 8 | 6.2546 |
| `sp-024` | `global` | 6/16 | 37.50% | 112 | 495 | 0 | 6 | 6.0947 |
| `sp-034` | `global` | 6/16 | 37.50% | 116 | 811 | 0 | 2 | 10.0307 |
| `sp-035` | `global` | 6/16 | 37.50% | 116 | 810 | 0 | 2 | 10.0184 |
| `sp-027` | `global` | 6/16 | 37.50% | 72 | 411 | 0 | 2 | 5.1107 |
| `sp-025` | `global` | 6/16 | 37.50% | 73 | 436 | 0 | 1 | 5.4305 |
| `sp-032` | `global` | 6/16 | 37.50% | 133 | 813 | 0 | 0 | 10.0799 |
| `sp-029` | `global` | 6/16 | 37.50% | 120 | 812 | 0 | 0 | 10.0676 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 1.38% | 6 | 1.38% | 725 | 100.00% | 8.9975 |
| `op-001` | 1 | 1.38% | 6 | 1.38% | 736 | 100.00% | 9.1328 |
| `op-002` | 1 | 1.38% | 6 | 1.38% | 178 | 100.00% | 2.2694 |
| `op-003` | 1 | 1.38% | 6 | 1.38% | 721 | 100.00% | 8.9483 |
| `op-004` | 1 | 1.38% | 6 | 1.38% | 216 | 100.00% | 2.7368 |
| `op-005` | 1 | 1.38% | 6 | 1.38% | 193 | 100.00% | 2.4539 |
| `op-006` | 1 | 1.38% | 6 | 1.38% | 194 | 100.00% | 2.4662 |
| `op-007` | 1 | 1.38% | 6 | 1.38% | 724 | 100.00% | 8.9852 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 3 | 0 | 0 | 0 | 0.0000 | 59.7600 | 3 saturated |
| 2 | 100.00% | 18 | 0 | 0 | 0 | 0.0000 | 59.7600 | 18 saturated |
| 3 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 59.7600 | steady state |
| 4 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 59.7600 | steady state |
| 5 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 59.7600 | steady state |
| 6 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 59.7600 | steady state |
| 7 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 59.7600 | steady state |
| 8 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 59.7600 | steady state |
...
The run minted `77.1200` reward/audit units and burned `41.2800` units, for a burn-to-mint ratio of `53.53%`.

Providers earned `579.8400` in modeled revenue against `101.7600` in modeled cost, ending with aggregate P&L `478.0800`.

Retrieval accounting paid providers `510.7200`, burned `14.4000` in base fees, and burned `26.8800` in variable retrieval fees.

Wash-retrieval accounting shows explicit spend `14.4000` against possible colluding-provider gain `496.3200`.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-002` | 6 | 0.9600 + 2.3674 | 1.0580 | 0.0000 | 2.2694 | no |
| `sp-019` | 6 | 0.9600 + 2.4073 | 1.0610 | 0.0000 | 2.3063 | no |
| `sp-022` | 6 | 0.9600 + 2.4206 | 1.0620 | 0.0000 | 2.3186 | no |
| `sp-017` | 6 | 0.9600 + 2.4605 | 1.0650 | 0.0000 | 2.3555 | no |
| `sp-005` | 6 | 0.9600 + 2.5669 | 1.0730 | 0.0000 | 2.4539 | no |
...
| `min_high_bandwidth_promotions` | `PASS` | Capability promotion: measured fast providers should become high-bandwidth eligible. | high_bandwidth_promotions=32, required>=1 |
| `min_high_bandwidth_demotions` | `PASS` | Custom assertion. Review the detail and fixture threshold. | high_bandwidth_demotions=2, required>=1 |
| `min_saturated_responses` | `PASS` | Scale fixture must expose provider bandwidth saturation. | saturated_responses=21, required>=1 |
| `min_hot_retrieval_attempts` | `PASS` | Hot-service fixture must exercise hot retrieval demand. | hot_retrieval_attempts=4800, required>=1 |
| `min_hot_high_bandwidth_serves` | `PASS` | Hot-service routing must use promoted high-bandwidth providers. | hot_high_bandwidth_serves=21062, required>=1 |
