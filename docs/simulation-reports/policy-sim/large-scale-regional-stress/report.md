...
Repair coordination was constrained: `12436` repair backoffs occurred across `16060` repair attempts. Cooldown backoffs accounted for `0` events and attempt-cap backoffs accounted for `0` events. Pending-provider readiness timeouts accounted for `0` events.
...
| Final repair backlog | `574` slots | Started repairs minus completed or timed-out repairs at run end. |
...
| Owner-funded attempts / owner escrow debit | `144000` / `0.0000` | Detects whether public demand is unexpectedly draining the deal owner's escrow. |
| Wash accounted spend / net gain | `288.0000` / `17500.5065` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
...
| Deals open/closed/expired | `1500` / `0` / `0` | Confirms close/refund/expiry semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
Wash-retrieval accounting shows explicit spend `288.0000` against possible colluding-provider gain `17500.5065`.
