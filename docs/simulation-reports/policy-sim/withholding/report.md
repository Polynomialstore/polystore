# Policy Simulation Report: Withholding Provider

## Executive Summary

**Verdict:** `PASS`. This run simulates `withholding` with `48` providers, `80` data users, `24` deals, and an RS `8+4` layout for `10` epochs. Enforcement is configured as `REWARD_EXCLUSION`.

Model a provider that stays nominally online but withholds retrieval responses and skips synthetic proof participation. This is a griefing case because the provider may try to appear present while forcing deputies or peers to absorb load.
...
Confirm that withholding is distinguishable from ordinary jitter before any real slashing policy is considered.
...
| Provider P&L p10 / p50 / p90 | `1.1115` / `1.2475` / `1.4180` | Shows whether aggregate P&L hides marginal-provider distress. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 48 | 37.50% | 0 | 0 | 1 | 1.2258 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-035` | `global` | 7/16 | 43.75% | 0 | 176 | 0 | 0 | 1.6560 |
| `sp-024` | `global` | 7/16 | 43.75% | 0 | 168 | 0 | 0 | 1.5880 |
| `sp-028` | `global` | 7/16 | 43.75% | 0 | 162 | 0 | 0 | 1.5370 |
| `sp-031` | `global` | 6/16 | 37.50% | 0 | 157 | 0 | 0 | 1.4345 |
| `sp-030` | `global` | 6/16 | 37.50% | 0 | 153 | 0 | 0 | 1.4005 |
| `sp-022` | `global` | 7/16 | 43.75% | 0 | 152 | 0 | 0 | 1.4520 |
| `sp-029` | `global` | 6/16 | 37.50% | 0 | 150 | 0 | 0 | 1.3750 |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-016` | 1 | 2.08% | 7 | 2.43% | 148 | 100.00% | 1.4180 |
| `op-022` | 1 | 2.08% | 7 | 2.43% | 152 | 100.00% | 1.4520 |
| `op-024` | 1 | 2.08% | 7 | 2.43% | 168 | 100.00% | 1.5880 |
| `op-028` | 1 | 2.08% | 7 | 2.43% | 162 | 100.00% | 1.5370 |
| `op-035` | 1 | 2.08% | 7 | 2.43% | 176 | 100.00% | 1.6560 |
| `op-039` | 1 | 2.08% | 7 | 2.43% | 140 | 100.00% | 1.3500 |
| `op-001` | 1 | 2.08% | 6 | 2.08% | 126 | 100.00% | 1.1710 |
| `op-002` | 1 | 2.08% | 6 | 2.08% | 136 | 100.00% | 1.2560 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 28 | 6 | 0 | 0 | 0.1200 | 5.8000 | 16 withheld, 6 quota misses, 6 delinquent slots |
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-000` | 0 | 0.0000 + 0.0000 | 0.6200 | 0.0000 | -0.6200 | yes |
| `sp-004` | 6 | 1.2000 + 1.0735 | 1.2130 | 0.0000 | 1.0605 | no |
| `sp-036` | 6 | 1.2000 + 1.0925 | 1.2150 | 0.0000 | 1.0775 | no |
| `sp-008` | 6 | 1.2000 + 1.1210 | 1.2180 | 0.0000 | 1.1030 | no |
| `sp-011` | 6 | 1.2000 + 1.1305 | 1.2190 | 0.0000 | 1.1115 | no |
...
| `min_withheld_responses` | `PASS` | Withholding fixture must create visible withheld-response evidence. | withheld_responses=16, required>=1 |
