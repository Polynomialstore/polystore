# Policy Simulation Report: Closed Retrieval Rejection

## Executive Summary

**Verdict:** `PASS`. This run simulates `closed-retrieval-rejection` with `48` providers, `80` data users, `12` deals, and an RS `8+4` layout for `6` epochs. Enforcement is configured as `REWARD_EXCLUSION`.

Model retrieval semantics after an intentional deal close and storage-escrow refund. The policy question is whether requests after close are counted as explicit closed-content rejections instead of availability failures or billable retrievals.

Expected policy behavior: Deals close cleanly, unearned storage escrow is refunded, later retrieval attempts are rejected as closed content, unavailable reads remain zero, and no retrieval escrow is debited after close.

Observed result: retrieval success was `100.00%`, reward coverage was `100.00%`, repairs started/ready/completed were `0` / `0` / `0`, and `0` providers ended with negative modeled P&L. The run recorded `0` unavailable reads, `0` expired retrieval rejections, `160` closed retrieval rejections, `0` modeled data-loss events, `0` bandwidth saturation responses and `0` repair backoffs across `0` repair attempts, with `0` pending-repair readiness timeouts. Slot health recorded `0` suspect slot-epochs and `0` delinquent slot-epochs. High-bandwidth promotions were `0` and final high-bandwidth providers were `0`.

## Review Focus

Use this before implementing gateway/keeper post-close query behavior, closed-deal response codes, and retrieval accounting guards.
...
| Storage lock-in | `true`; duration `6` epochs |
| Deal expiry | `false` |
| Deal close policy | epoch `4`; count `12`; share `0.00%` |
...
| Storage lock-in | enabled `True`, duration `6` epochs | If enabled, committed deals lock storage escrow upfront at the quoted storage price and earn it over the modeled duration. |
| Deal expiry | enabled `False` | If enabled, deals auto-expire once their modeled duration has fully earned. |
| Deal close/refund | epoch `4`, count `12`, share `0.00%` | Optional early close refunds unearned storage escrow and removes closed deals from active responsibility. |
...
Post-close retrieval behavior was exercised: `160` requests arrived after all active deals had been intentionally closed. The simulator counted them as explicit closed-content rejections, not live availability failures or billable retrieval attempts.

The policy layer recorded `24` evidence events: `0` soft, `0` threshold, `0` hard, `0` economic, `24` market, `0` spam, and `0` operational events. Soft and economic evidence are suitable for repair and reward exclusion; hard or convicted threshold evidence is the category that can later justify slashing or stronger sanctions.

Storage escrow lifecycle accounting was exercised: `691.2000` was locked for committed storage, `460.8000` was earned over modeled service epochs, `230.4000` was refunded on close, and final outstanding storage escrow was `0.0000`. Closed deals ended at `12`, expired deals ended at `0`, and open deals ended at `0`.
...
| Closed retrieval rejections | `160` | Post-close requests should be rejected explicitly instead of counted as live availability failures or billable retrievals. |
...
| Owner-funded attempts / owner escrow debit | `320` / `0.0000` | Detects whether public demand is unexpectedly draining the deal owner's escrow. |
| Wash accounted spend / net gain | `0.3200` / `24.0000` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
| Storage escrow locked/earned/refunded | `691.2000` / `460.8000` / `230.4000` | Shows quote-to-lock, provider earning, and close/refund accounting for committed storage. |
| Storage escrow outstanding | `0.0000` final; peak `576.0000` | Detects funds left locked after close/expiry semantics should have released them. |
| Storage fee provider payout/burned | `460.8000` / `0.0000` | Separates earned storage fees paid to eligible providers from fees withheld from non-compliant responsibility. |
| Deals open/closed/expired | `0` / `12` / `0` | Confirms close/refund/expiry semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
| Top operator assignment share | `0.00%` | Shows whether placement caps translate identity concentration into slot concentration. |
...
| Final storage utilization | `0.00%` | Active slots versus modeled provider capacity. |
| Provider utilization p50 / p90 / max | `0.00%` / `0.00%` / `0.00%` | Detects assignment concentration and capacity cliffs. |
| Provider P&L p10 / p50 / p90 | `10.1280` / `10.1790` / `10.2300` | Shows whether aggregate P&L hides marginal-provider distress. |
...
| Retrieval latent/effective attempts | `480` / `320` | Shows how much retrieval load was added by demand-shock multipliers. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 48 | 0.00% | 0 | 0 | 0 | 10.1733 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-040` | `global` | 0/16 | 0.00% | 0 | 63 | 0 | 0 | 10.2555 |
| `sp-001` | `global` | 0/16 | 0.00% | 0 | 61 | 0 | 0 | 10.2385 |
| `sp-024` | `global` | 0/16 | 0.00% | 0 | 61 | 0 | 0 | 10.2385 |
| `sp-030` | `global` | 0/16 | 0.00% | 0 | 60 | 0 | 0 | 10.2300 |
| `sp-035` | `global` | 0/16 | 0.00% | 0 | 60 | 0 | 0 | 10.2300 |
| `sp-039` | `global` | 0/16 | 0.00% | 0 | 60 | 0 | 0 | 10.2300 |
| `sp-041` | `global` | 0/16 | 0.00% | 0 | 60 | 0 | 0 | 10.2300 |
| `sp-043` | `global` | 0/16 | 0.00% | 0 | 60 | 0 | 0 | 10.2300 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 2.08% | 0 | 0.00% | 51 | 100.00% | 10.1535 |
| `op-001` | 1 | 2.08% | 0 | 0.00% | 61 | 100.00% | 10.2385 |
| `op-002` | 1 | 2.08% | 0 | 0.00% | 56 | 100.00% | 10.1960 |
| `op-003` | 1 | 2.08% | 0 | 0.00% | 56 | 100.00% | 10.1960 |
| `op-004` | 1 | 2.08% | 0 | 0.00% | 53 | 100.00% | 10.1705 |
| `op-005` | 1 | 2.08% | 0 | 0.00% | 57 | 100.00% | 10.2045 |
| `op-006` | 1 | 2.08% | 0 | 0.00% | 57 | 100.00% | 10.2045 |
| `op-007` | 1 | 2.08% | 0 | 0.00% | 49 | 100.00% | 10.1365 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 122.3200 | steady state |
...
| 5 | 0.00% | 0 | 0 | 0 | 0 | 0.0000 | -0.4800 | steady state |
| 6 | 0.00% | 0 | 0 | 0 | 0 | 0.0000 | -0.4800 | steady state |

## Enforcement Interpretation

The simulator recorded `24` evidence events and `0` repair ledger events. The first evidence epoch was `1` and the first repair-start epoch was `none`.

Evidence by reason:

- `storage_escrow_locked`: `12`
- `deal_storage_escrow_closed`: `12`
...
- Final active slots in last epoch: `0`
...
The run minted `17.5200` reward/audit units and burned `1.6000` units, for a burn-to-mint ratio of `9.13%`.

Providers earned `496.6400` in modeled revenue against `8.3200` in modeled cost, ending with aggregate P&L `488.3200`.

Retrieval accounting paid providers `24.3200`, burned `0.3200` in base fees, and burned `1.2800` in variable retrieval fees.

Wash-retrieval accounting shows explicit spend `0.3200` against possible colluding-provider gain `24.0000`.

Sponsored retrieval accounting spent `0.0000` across `0` sponsor-funded attempts; owner retrieval escrow debit was `0.0000`.

Storage escrow accounting locked `691.2000`, earned `460.8000`, refunded `230.4000`, paid providers `460.8000`, burned `0.0000`, and ended with outstanding escrow `0.0000`.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-022` | 0 | 0.2400 + 0.3325 | 0.1550 | 0.0000 | 10.0175 | no |
| `sp-021` | 0 | 0.2400 + 0.4085 | 0.1630 | 0.0000 | 10.0855 | no |
| `sp-012` | 0 | 0.2400 + 0.4275 | 0.1650 | 0.0000 | 10.1025 | no |
| `sp-016` | 0 | 0.2400 + 0.4275 | 0.1650 | 0.0000 | 10.1025 | no |
| `sp-017` | 0 | 0.2400 + 0.4465 | 0.1670 | 0.0000 | 10.1195 | no |
...
| `min_closed_retrieval_attempts` | `PASS` | Post-close behavior: intentionally closed content requests must be counted separately from live availability failures. | closed_retrieval_attempts=160, required>=160 |
...
| `min_storage_escrow_refunded` | `PASS` | Early-close fixture must refund unearned storage escrow. | storage_escrow_refunded=230.4, required>=200 |
...
| `exact_final_closed_deals` | `PASS` | Close/refund fixture should end with the configured number of closed deals. | final_closed_deals=12, required=12 |
| `exact_deals_closed` | `PASS` | Close/refund fixture should close the configured number of deals. | deals_closed=12, required=12 |
...
| 1 | 10 |  | `user-gateway` | `market` | `storage_escrow_locked` | `storage_lockin` |
| 1 | 11 |  | `user-gateway` | `market` | `storage_escrow_locked` | `storage_lockin` |
| 1 | 12 |  | `user-gateway` | `market` | `storage_escrow_locked` | `storage_lockin` |
| ... | ... | ... | ... | ... | ... | `12` more events omitted |
