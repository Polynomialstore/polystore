...
Observed result: retrieval success was `100.00%`, reward coverage was `98.11%`, repairs started/ready/completed were `32` / `32` / `32`, and `4` providers ended with negative modeled P&L. The run recorded `0` unavailable reads, `0` expired retrieval rejections, `0` closed retrieval rejections, `0` modeled data-loss events, `0` bandwidth saturation responses and `0` repair backoffs across `32` repair attempts, with `0` pending-repair readiness timeouts. Slot health recorded `32` suspect slot-epochs and `64` delinquent slot-epochs. High-bandwidth promotions were `0` and final high-bandwidth providers were `0`.
...
| Owner-funded attempts / owner escrow debit | `960` / `0.0000` | Detects whether public demand is unexpectedly draining the deal owner's escrow. |
| Wash accounted spend / net gain | `0.9600` / `72.0000` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
...
| Churn pressure provider-epochs / peak | `32` / `4` | Shows the breadth and duration of providers below the configured churn threshold. |
| Active / exited / reserve provider capacity | `960` / `0` / `0` slots | Measures supply remaining, removed, and still waiting outside normal placement. |
...
| Retrieval latent/effective attempts | `960` / `960` | Shows how much retrieval load was added by demand-shock multipliers. |
...
Wash-retrieval accounting shows explicit spend `0.9600` against possible colluding-provider gain `72.0000`.
