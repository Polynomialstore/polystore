# Policy Simulation Report: Staged Upload Grief

## Executive Summary

**Verdict:** `PASS`. This run simulates `staged-upload-grief` with `48` providers, `80` data users, `24` deals, and an RS `8+4` layout for `8` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Expected policy behavior: Preflight rejections and retention cleanup are visible, pending provisional generations stay under the configured cap, no repair starts, and committed retrieval availability remains intact.
...
Use this before implementing provider-daemon staged-generation GC, gateway preflight checks, and operator alerts for abandoned provisional data.
...
| Staged uploads/epoch | `30` provisional attempts |
| Staged upload retention | `2` epochs (`0` disables age cleanup) |
| Staged upload pending cap | `36` generations (`0` means unlimited) |
...
| Staged upload attempts/epoch | `30` | Provisional generations that consume local provider-daemon staging space before content commit. |
| Staged upload commit rate | `0.00%` | Share of provisional uploads that become committed content instead of remaining abandoned local state. |
| Staged upload retention/cap | `2` epochs / `36` generations | Local cleanup and preflight limits used to bound abandoned provisional-generation storage pressure. |
...
The policy layer recorded `10` evidence events: `0` soft, `0` threshold, `0` hard, `0` economic, `0` market, `0` spam, and `10` operational events. Soft and economic evidence are suitable for repair and reward exclusion; hard or convicted threshold evidence is the category that can later justify slashing or stronger sanctions.

Staged upload grief was exercised: `240` provisional upload attempts produced `96` preflight rejections and `108` retention cleanup events. Peak pending provisional state was `36` generations / `72` MDUs, ending at `36` generations.
...
| Staged upload attempts/accepted/committed | `240` / `144` / `0` | Shows provisional upload pressure separately from committed storage demand. |
| Staged upload rejections/cleaned | `96` / `108` | Preflight rejection and retention cleanup should bound abandoned provisional generations. |
| Staged pending generations/MDUs peak | `36` / `72` | Detects whether local staged storage pressure exceeded configured caps. |
...
| Provider P&L p10 / p50 / p90 | `0.8790` / `0.9980` / `1.0745` | Shows whether aggregate P&L hides marginal-provider distress. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 48 | 37.50% | 0 | 0 | 0 | 0.9867 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-017` | `global` | 6/16 | 37.50% | 0 | 121 | 0 | 0 | 1.1085 |
| `sp-020` | `global` | 6/16 | 37.50% | 0 | 121 | 0 | 0 | 1.1085 |
| `sp-021` | `global` | 6/16 | 37.50% | 0 | 121 | 0 | 0 | 1.1085 |
| `sp-013` | `global` | 6/16 | 37.50% | 0 | 120 | 0 | 0 | 1.1000 |
| `sp-041` | `global` | 6/16 | 37.50% | 0 | 119 | 0 | 0 | 1.0915 |
| `sp-047` | `global` | 6/16 | 37.50% | 0 | 117 | 0 | 0 | 1.0745 |
| `sp-023` | `global` | 6/16 | 37.50% | 0 | 116 | 0 | 0 | 1.0660 |
| `sp-044` | `global` | 6/16 | 37.50% | 0 | 115 | 0 | 0 | 1.0575 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 2.08% | 6 | 2.08% | 93 | 100.00% | 0.8705 |
| `op-001` | 1 | 2.08% | 6 | 2.08% | 103 | 100.00% | 0.9555 |
| `op-002` | 1 | 2.08% | 6 | 2.08% | 109 | 100.00% | 1.0065 |
| `op-003` | 1 | 2.08% | 6 | 2.08% | 101 | 100.00% | 0.9385 |
| `op-004` | 1 | 2.08% | 6 | 2.08% | 94 | 100.00% | 0.8790 |
| `op-005` | 1 | 2.08% | 6 | 2.08% | 95 | 100.00% | 0.8875 |
| `op-006` | 1 | 2.08% | 6 | 2.08% | 109 | 100.00% | 1.0065 |
| `op-007` | 1 | 2.08% | 6 | 2.08% | 90 | 100.00% | 0.8450 |
...
| 2 | 100.00% | 24 | 0 | 0 | 0 | 0.0000 | 5.9200 | 24 staged preflight rejections |
| 3 | 100.00% | 30 | 0 | 0 | 0 | 0.0000 | 5.9200 | 30 staged generations cleaned |
| 4 | 100.00% | 30 | 0 | 0 | 0 | 0.0000 | 5.9200 | 24 staged preflight rejections, 6 staged generations cleaned |
| 5 | 100.00% | 30 | 0 | 0 | 0 | 0.0000 | 5.9200 | 30 staged generations cleaned |
| 6 | 100.00% | 30 | 0 | 0 | 0 | 0.0000 | 5.9200 | 24 staged preflight rejections, 6 staged generations cleaned |
| 7 | 100.00% | 30 | 0 | 0 | 0 | 0.0000 | 5.9200 | 30 staged generations cleaned |
| 8 | 100.00% | 30 | 0 | 0 | 0 | 0.0000 | 5.9200 | 24 staged preflight rejections, 6 staged generations cleaned |

## Enforcement Interpretation

The simulator recorded `10` evidence events and `0` repair ledger events. The first evidence epoch was `2` and the first repair-start epoch was `none`.

Evidence by reason:

- `staged_upload_retention_cleanup`: `6`
- `staged_upload_preflight_rejected`: `4`
...
The run minted `54.0800` reward/audit units and burned `3.2000` units, for a burn-to-mint ratio of `5.92%`.

Providers earned `94.7200` in modeled revenue against `47.3600` in modeled cost, ending with aggregate P&L `47.3600`.
...
Staged upload accounting saw `240` provisional attempts, accepted `144`, committed `0`, rejected `96` at preflight, and cleaned `108` abandoned generations by retention policy. The peak local staged footprint was `36` generations / `72` MDUs.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-007` | 6 | 0.9600 + 0.8550 | 0.9700 | 0.0000 | 0.8450 | no |
| `sp-011` | 6 | 0.9600 + 0.8740 | 0.9720 | 0.0000 | 0.8620 | no |
| `sp-000` | 6 | 0.9600 + 0.8835 | 0.9730 | 0.0000 | 0.8705 | no |
| `sp-004` | 6 | 0.9600 + 0.8930 | 0.9740 | 0.0000 | 0.8790 | no |
| `sp-008` | 6 | 0.9600 + 0.8930 | 0.9740 | 0.0000 | 0.8790 | no |
...
| `min_staged_upload_attempts` | `PASS` | Staged upload fixture must exercise provisional generation pressure. | staged_upload_attempts=240, required>=240 |
| `min_staged_upload_rejections` | `PASS` | Staged upload grief must hit preflight rejection once pending provisional state reaches the cap. | staged_upload_rejections=96, required>=1 |
| `min_staged_upload_cleaned` | `PASS` | Staged upload retention must clean abandoned provisional generations. | staged_upload_cleaned=108, required>=1 |
| `max_max_staged_upload_pending_generations` | `PASS` | Staged upload pending generations must stay below the configured cap. | max_staged_upload_pending_generations=36, required<=36 |
| `max_max_staged_upload_pending_mdus` | `PASS` | Staged upload pending MDU footprint must stay below the configured cap. | max_staged_upload_pending_mdus=72, required<=72 |
| `max_final_staged_upload_pending_generations` | `PASS` | Staged upload final pending generation count should remain bounded at run end. | final_staged_upload_pending_generations=36, required<=36 |
...
|---:|---:|---:|---|---|---|---|
| 2 |  |  | `user-gateway` | `operational` | `staged_upload_preflight_rejected` | `retention_limit` |
| 3 |  |  | `provider-daemon` | `operational` | `staged_upload_retention_cleanup` | `local_gc` |
| 4 |  |  | `user-gateway` | `operational` | `staged_upload_preflight_rejected` | `retention_limit` |
| 4 |  |  | `provider-daemon` | `operational` | `staged_upload_retention_cleanup` | `local_gc` |
| 5 |  |  | `provider-daemon` | `operational` | `staged_upload_retention_cleanup` | `local_gc` |
| 6 |  |  | `user-gateway` | `operational` | `staged_upload_preflight_rejected` | `retention_limit` |
| 6 |  |  | `provider-daemon` | `operational` | `staged_upload_retention_cleanup` | `local_gc` |
| 7 |  |  | `provider-daemon` | `operational` | `staged_upload_retention_cleanup` | `local_gc` |
| 8 |  |  | `user-gateway` | `operational` | `staged_upload_preflight_rejected` | `retention_limit` |
| 8 |  |  | `provider-daemon` | `operational` | `staged_upload_retention_cleanup` | `local_gc` |
