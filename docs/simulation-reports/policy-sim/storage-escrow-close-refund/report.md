# Policy Simulation Report: Storage Escrow Close And Refund

## Executive Summary

**Verdict:** `PASS`. This run simulates `storage-escrow-close-refund` with `48` providers, `80` data users, `12` deals, and an RS `8+4` layout for `8` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Use this before implementing keeper close/refund semantics, quote-to-charge parity, storage-fee payout, and end-of-deal accounting tests.
...
| Deal close policy | epoch `4`; count `4`; share `0.00%` |
...
| Deal close/refund | epoch `4`, count `4`, share `0.00%` | Optional early close refunds unearned storage escrow and removes closed deals from active responsibility. |
...
Storage escrow lifecycle accounting was exercised: `921.6000` was locked for committed storage, `768.0000` was earned over modeled service epochs, `153.6000` was refunded on close, and final outstanding storage escrow was `0.0000`. Closed deals ended at `4` and open deals ended at `8`.
...
| Deals open/closed | `8` / `4` | Confirms close/refund semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
The simulator recorded `16` evidence events and `0` repair ledger events. The first evidence epoch was `1` and the first repair-start epoch was `none`.

Evidence by reason:

- `storage_escrow_locked`: `12`
- `deal_storage_escrow_closed`: `4`
...
Storage escrow accounting locked `921.6000`, earned `768.0000`, refunded `153.6000`, paid providers `768.0000`, burned `0.0000`, and ended with outstanding escrow `0.0000`.
...
| `min_storage_escrow_earned` | `PASS` | Storage lock-in fixture must earn storage fees over modeled service epochs. | storage_escrow_earned=768, required>=760 |
| `min_storage_escrow_refunded` | `PASS` | Early-close fixture must refund unearned storage escrow. | storage_escrow_refunded=153.6, required>=150 |
