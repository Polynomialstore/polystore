# Policy Simulation Report: Provider Cost Shock

## Executive Summary

**Verdict:** `PASS`. This run simulates `provider-cost-shock` with `64` providers, `80` data users, `40` deals, and an RS `8+4` layout for `10` epochs. Enforcement is configured as `REWARD_EXCLUSION`.
...
Expected policy behavior: Retrieval success remains high, the cost-shock window is visible, provider P&L falls, and negative-P&L churn risk appears.

Observed result: retrieval success was `100.00%`, reward coverage was `100.00%`, repairs started/ready/completed were `0` / `0` / `0`, and `64` providers ended with negative modeled P&L. The run recorded `0` unavailable reads, `0` expired retrieval rejections, `0` closed retrieval rejections, `0` modeled data-loss events, `0` bandwidth saturation responses and `0` repair backoffs across `0` repair attempts, with `0` pending-repair readiness timeouts. Slot health recorded `0` suspect slot-epochs and `0` delinquent slot-epochs. High-bandwidth promotions were `0` and final high-bandwidth providers were `0`.

## Review Focus

Use this fixture to calibrate cost assumptions, price floors, reward buffers, and whether pricing should react to provider cost telemetry.
...
| Wash accounted spend / net gain | `0.8000` / `53.5966` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
...
| Deals open/closed/expired | `40` / `0` / `0` | Confirms close/refund/expiry semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
| Top operator assignment share | `1.66%` | Shows whether placement caps translate identity concentration into slot concentration. |
...
| Churn pressure provider-epochs / peak | `223` / `64` | Shows the breadth and duration of providers below the configured churn threshold. |
| Active / exited / reserve provider capacity | `1024` / `0` / `0` slots | Measures supply remaining, removed, and still waiting outside normal placement. |
| Peak assigned slots on churned providers | `0` | Shows the maximum repair burden created by economic exits. |
| Storage price start/end/range | `1.0000` -> `1.2489` (`1.0000`-`1.2489`) | Shows dynamic pricing movement and bounds. |
| Retrieval price start/end/range | `0.0100` -> `0.0080` (`0.0080`-`0.0100`) | Shows whether demand pressure moved retrieval pricing. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 64 | 46.87% | 0 | 0 | 64 | -0.7101 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-025` | `global` | 8/16 | 50.00% | 0 | 130 | 0 | 0 | -0.5123 |
| `sp-020` | `global` | 8/16 | 50.00% | 0 | 119 | 0 | 0 | -0.5823 |
| `sp-026` | `global` | 8/16 | 50.00% | 0 | 119 | 0 | 0 | -0.5891 |
| `sp-024` | `global` | 8/16 | 50.00% | 0 | 118 | 0 | 0 | -0.5887 |
| `sp-028` | `global` | 8/16 | 50.00% | 0 | 118 | 0 | 0 | -0.6014 |
| `sp-027` | `global` | 8/16 | 50.00% | 0 | 117 | 0 | 0 | -0.5974 |
| `sp-002` | `global` | 8/16 | 50.00% | 0 | 115 | 0 | 0 | -0.6333 |
| `sp-029` | `global` | 8/16 | 50.00% | 0 | 114 | 0 | 0 | -0.6312 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 1.56% | 8 | 1.66% | 104 | 100.00% | -0.6981 |
| `op-001` | 1 | 1.56% | 8 | 1.66% | 100 | 100.00% | -0.7410 |
| `op-002` | 1 | 1.56% | 8 | 1.66% | 115 | 100.00% | -0.6333 |
| `op-003` | 1 | 1.56% | 8 | 1.66% | 113 | 100.00% | -0.6375 |
| `op-004` | 1 | 1.56% | 8 | 1.66% | 98 | 100.00% | -0.7571 |
| `op-005` | 1 | 1.56% | 8 | 1.66% | 90 | 100.00% | -0.8031 |
| `op-006` | 1 | 1.56% | 8 | 1.66% | 88 | 100.00% | -0.8051 |
| `op-007` | 1 | 1.56% | 8 | 1.66% | 98 | 100.00% | -0.7405 |

### Timeline

| Epoch | Retrieval Success | Evidence | Repairs Started | Repairs Ready | Repairs Completed | Reward Burned | Provider P&L | Notes |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 11.8400 | steady state |
| 2 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 11.6880 | steady state |
| 3 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 11.5398 | steady state |
| 4 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | 11.3953 | steady state |
| 5 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | -14.9856 | steady state |
| 6 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | -15.1229 | steady state |
| 7 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | -15.2569 | steady state |
| 8 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | -15.3874 | steady state |
| 9 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | -15.5148 | steady state |
| 10 | 100.00% | 0 | 0 | 0 | 0 | 0.0000 | -15.6389 | steady state |
...
The run minted `154.0000` reward/audit units and burned `3.6630` units, for a burn-to-mint ratio of `2.38%`.

Providers earned `198.3966` in modeled revenue against `243.8400` in modeled cost, ending with aggregate P&L `-45.4434`.

Retrieval accounting paid providers `54.3966`, burned `0.8000` in base fees, and burned `2.8630` in variable retrieval fees.

Wash-retrieval accounting shows explicit spend `0.8000` against possible colluding-provider gain `53.5966`.
...
Provider cost shocks were active for `6` shock-epochs, affecting up to `64` providers. The maximum modeled storage-cost multiplier reached `500.00%`.

`64` providers ended with negative P&L and `64` were marked as churn risk. That is economically important even when retrieval success is perfect.

Final modeled storage price was `1.2489` and retrieval price per slot was `0.0080`.

### Provider P&L Extremes

| Provider | Assigned Slots | Revenue | Cost | Slashed | P&L | Churn Risk |
|---|---:|---:|---:|---:|---:|---:|
| `sp-063` | 7 | 2.1000 + 0.6758 | 3.6090 | 0.0000 | -0.8332 | yes |
| `sp-043` | 7 | 2.1000 + 0.6677 | 3.6000 | 0.0000 | -0.8323 | yes |
| `sp-061` | 7 | 2.1000 + 0.6834 | 3.6030 | 0.0000 | -0.8196 | yes |
| `sp-010` | 8 | 2.4000 + 0.7402 | 3.9570 | 0.0000 | -0.8168 | yes |
| `sp-014` | 8 | 2.4000 + 0.7548 | 3.9660 | 0.0000 | -0.8112 | yes |
...
| `min_provider_cost_shock_active` | `PASS` | Cost-shock fixture must activate the configured cost-pressure window. | provider_cost_shock_active=6, required>=6 |
| `min_max_provider_cost_shocked_providers` | `PASS` | Cost-shock fixture must affect at least this many providers. | max_provider_cost_shocked_providers=64, required>=64 |
| `min_max_provider_cost_shock_storage_multiplier_bps` | `PASS` | Cost-shock fixture must raise modeled storage cost by at least this multiplier. | max_provider_cost_shock_storage_multiplier_bps=50000, required>=50000 |
| `min_providers_negative_pnl` | `PASS` | Market warning: some providers must become economically distressed. | providers_negative_pnl=64, required>=1 |
