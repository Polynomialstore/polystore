...
| Owner-funded attempts / owner escrow debit | `22400` / `0.0000` | Detects whether public demand is unexpectedly draining the deal owner's escrow. |
| Wash accounted spend / net gain | `22.4000` / `2039.6559` | Worst-case colluding requester/provider economics after explicit base, sponsor, and owner-funded variable spend. |
...
| Deals open/closed/expired | `96` / `0` / `0` | Confirms close/refund/expiry semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
Wash-retrieval accounting shows explicit spend `22.4000` against possible colluding-provider gain `2039.6559`.
