# Policy Simulation Report: Storage Escrow Expiry

## Executive Summary

**Verdict:** `PASS`. This run simulates `storage-escrow-expiry` with `48` providers, `80` data users, `12` deals, and an RS `8+4` layout for `4` epochs. Enforcement is configured as `REWARD_EXCLUSION`.

Model end-of-duration storage accounting. The policy question is whether a fully earned deal auto-expires, leaves no hidden escrow, and stops contributing active responsibility after its duration.

Expected policy behavior: Storage escrow locks, earns exactly through the configured duration, every deal expires, no unearned refund is needed, and final open deals reach zero.
...
Use this before implementing keeper expiry auto-close, deal GC, and final payout/escrow settlement tests.
...
| Storage lock-in | `true`; duration `4` epochs |
| Deal expiry | `true` |
...
| Storage lock-in | enabled `True`, duration `4` epochs | If enabled, committed deals lock storage escrow upfront at the quoted storage price and earn it over the modeled duration. |
| Deal expiry | enabled `True` | If enabled, deals auto-expire once their modeled duration has fully earned. |
...
Storage escrow lifecycle accounting was exercised: `460.8000` was locked for committed storage, `460.8000` was earned over modeled service epochs, `0.0000` was refunded on close, and final outstanding storage escrow was `0.0000`. Closed deals ended at `12`, expired deals ended at `12`, and open deals ended at `0`.
...
| Storage escrow locked/earned/refunded | `460.8000` / `460.8000` / `0.0000` | Shows quote-to-lock, provider earning, and close/refund accounting for committed storage. |
| Storage escrow outstanding | `0.0000` final; peak `345.6000` | Detects funds left locked after close/expiry semantics should have released them. |
...
| Deals open/closed/expired | `0` / `12` / `12` | Confirms close/refund/expiry semantics remove deals from active responsibility instead of continuing to accrue rewards. |
...
| Provider P&L p10 / p50 / p90 | `10.1140` / `10.1905` / `10.2585` | Shows whether aggregate P&L hides marginal-provider distress. |
...
| Retrieval latent/effective attempts | `320` / `320` | Shows how much retrieval load was added by demand-shock multipliers. |
...
|---|---:|---:|---:|---:|---:|---:|
| `global` | 48 | 0.00% | 0 | 0 | 0 | 10.1933 |

### Top Bottleneck Providers

| Provider | Region | Slots/Capacity | Utilization | Bandwidth Cap | Attempts | Offline | Saturated | P&L |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `sp-006` | `global` | 0/16 | 0.00% | 0 | 68 | 0 | 0 | 10.3180 |
| `sp-005` | `global` | 0/16 | 0.00% | 0 | 64 | 0 | 0 | 10.2840 |
| `sp-010` | `global` | 0/16 | 0.00% | 0 | 63 | 0 | 0 | 10.2755 |
| `sp-011` | `global` | 0/16 | 0.00% | 0 | 62 | 0 | 0 | 10.2670 |
| `sp-004` | `global` | 0/16 | 0.00% | 0 | 61 | 0 | 0 | 10.2585 |
| `sp-008` | `global` | 0/16 | 0.00% | 0 | 61 | 0 | 0 | 10.2585 |
| `sp-009` | `global` | 0/16 | 0.00% | 0 | 61 | 0 | 0 | 10.2585 |
| `sp-012` | `global` | 0/16 | 0.00% | 0 | 61 | 0 | 0 | 10.2585 |

### Top Operators

| Operator | Providers | Provider Share | Assigned Slots | Assignment Share | Retrieval Attempts | Success | P&L |
|---|---:|---:|---:|---:|---:|---:|---:|
| `op-000` | 1 | 2.08% | 0 | 0.00% | 59 | 100.00% | 10.2415 |
| `op-001` | 1 | 2.08% | 0 | 0.00% | 52 | 100.00% | 10.1820 |
| `op-002` | 1 | 2.08% | 0 | 0.00% | 59 | 100.00% | 10.2415 |
| `op-003` | 1 | 2.08% | 0 | 0.00% | 60 | 100.00% | 10.2500 |
| `op-004` | 1 | 2.08% | 0 | 0.00% | 61 | 100.00% | 10.2585 |
| `op-005` | 1 | 2.08% | 0 | 0.00% | 64 | 100.00% | 10.2840 |
| `op-006` | 1 | 2.08% | 0 | 0.00% | 68 | 100.00% | 10.3180 |
| `op-007` | 1 | 2.08% | 0 | 0.00% | 58 | 100.00% | 10.2330 |
...
- `deal_storage_escrow_expired`: `12`
...
The run minted `15.5200` reward/audit units and burned `1.6000` units, for a burn-to-mint ratio of `10.31%`.

Providers earned `496.6400` in modeled revenue against `7.3600` in modeled cost, ending with aggregate P&L `489.2800`.
...
Storage escrow accounting locked `460.8000`, earned `460.8000`, refunded `0.0000`, paid providers `460.8000`, burned `0.0000`, and ended with outstanding escrow `0.0000`.
...
|---|---:|---:|---:|---:|---:|---:|
| `sp-025` | 0 | 0.2400 + 0.3705 | 0.1390 | 0.0000 | 10.0715 | no |
| `sp-035` | 0 | 0.2400 + 0.4085 | 0.1430 | 0.0000 | 10.1055 | no |
| `sp-026` | 0 | 0.2400 + 0.4180 | 0.1440 | 0.0000 | 10.1140 | no |
| `sp-029` | 0 | 0.2400 + 0.4180 | 0.1440 | 0.0000 | 10.1140 | no |
| `sp-031` | 0 | 0.2400 + 0.4180 | 0.1440 | 0.0000 | 10.1140 | no |
...
| `min_storage_escrow_locked` | `PASS` | Storage lock-in fixture must lock non-zero storage escrow at commit. | storage_escrow_locked=460.8, required>=450 |
| `min_storage_escrow_earned` | `PASS` | Storage lock-in fixture must earn storage fees over modeled service epochs. | storage_escrow_earned=460.8, required>=450 |
...
| `min_storage_fee_provider_payouts` | `PASS` | Earned storage fees should pay eligible providers. | storage_fee_provider_payouts=460.8, required>=450 |
...
| `exact_final_expired_deals` | `PASS` | Run-end expired deal count should match the scenario contract. | final_expired_deals=12, required=12 |
| `exact_deals_expired` | `PASS` | Deal expiry fixture should expire the expected number of deals. | deals_expired=12, required=12 |
