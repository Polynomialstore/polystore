...
| Elasticity (saturation signal → pre-emptive scaling) | PARTIAL | `polystorechain/proto/polystorechain/polystorechain/v1/tx.proto` (`MsgSignalSaturation`); `polystorechain/x/polystorechain/keeper/msg_server.go` (`SignalSaturation`); `types.VirtualStripe` + `Keeper.VirtualStripes` persist overlay route state; `GetVirtualStripe` / `ListVirtualStripesByDeal` expose that state | `polystorechain/x/polystorechain/keeper/msg_server_test.go` (`TestSignalSaturation`); `polystorechain/x/polystorechain/keeper/virtual_stripe_query_test.go` (`TestVirtualStripeQueries`) | Overlay replicas are now explicit state, but readiness, TTL cleanup, service accounting, `mode2_slots` interaction, and gateway route selection are not implemented; no overlay-serving E2E coverage; treat as experimental / dev-only. |
