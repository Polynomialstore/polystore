# PolyStore Gateway GUI (Local Gateway)

`polystore_gateway_gui` is the desktop local Gateway app for browser users on trusted devnet. It runs a local PolyStore **user-gateway** on `http://localhost:8080` so the website can use local routing/cache flows without relying on a hosted relay gateway.
...
1. Download and install the latest GUI build for your OS from GitHub Releases.
   - macOS: choose the latest `.dmg` for Apple Silicon or Intel, open it, copy `polystore_gateway_gui.app` into `/Applications` if needed, then launch it with `open /Applications/polystore_gateway_gui.app` or from Finder.
   - Linux: choose the latest `.deb` on Ubuntu or Debian, or `.rpm` on RPM-based systems, install it, then launch `polystore_gateway_gui` from the app menu or by running `polystore_gateway_gui`.
2. Start PolyStore Gateway GUI (it auto-checks localhost and auto-starts the local Gateway if needed).
...
(`$ORIGIN` / `@loader_path`) into `nil_gateway` so it can find `libpolystore_core`
