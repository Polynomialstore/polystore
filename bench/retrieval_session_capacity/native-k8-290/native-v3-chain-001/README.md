...
Raw source hashes and published file hashes are listed in [manifest.json](manifest.json). Raw evidence, refund transactions, proof inventory, provider/validator logs, keyrings and generated payloads remain private. The extractor pins the driver and all three imported helper modules before executing their verified bytes, without adding the supplied directory to Python's import paths. It checks the original evidence and reconciled-block hashes before deriving retained metrics, after semantic validation. It checks successful completion, workload counts, signer/transaction identities, authoritative bitmaps, per-session expiry and refunds with four-validator receipts and recovered messages, block hashes and totals, gas simulations, CPU and resource records. Its runnable check compares the complete derived summary with the published JSON after the documented harness-path substitution, verifies all six published file hashes, accepts this retained success, rejects 40 altered inputs, and prevents sibling modules from shadowing standard-library imports. Failed earlier correctness smokes remain failures and are excluded from this report. This publication changes only the artifact directory; collection harness and runtime bytes remain frozen.
