...
func TestHistoricalRetrievalProofRequiresAuthorizedSigner(t *testing.T) {
	f := initFixture(t)
	msgServer := keeper.NewMsgServerImpl(f.keeper)
	...
	unrelatedBytes := make([]byte, 20)
	unrelatedBytes[0] = 99
	unrelated := sdk.AccAddress(unrelatedBytes).String()
	_, err = msgServer.RegisterProvider(f.ctx, &types.MsgRegisterProvider{Creator: unrelated, Capabilities: "General", TotalStorage: 100000000000, Endpoints: testProviderEndpoints})
	require.NoError(t, err)
	_, err = msgServer.SubmitRetrievalSessionProof(f.ctx, &types.MsgSubmitRetrievalSessionProof{
		Creator:   unrelated,
		SessionId: openRes.SessionId,
	...
	require.Error(t, err, "valid public proof must not authorize an unrelated registered signer")
}
