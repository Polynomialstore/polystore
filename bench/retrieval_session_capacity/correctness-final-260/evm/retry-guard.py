...
args=['go','test','-mod=readonly','-p=2','-overlay='+str(r/'overlay.json'),'./app','-run','TestHistoricalCaughtChildRollback','-count=1','-timeout=90s','-v']
...
with (r/'rollback-retry.log').open('w') as log:
...
(r/'retry-guard.json').write_text(json.dumps(report,indent=2)+'\n')
