...
**Description:** Develop a `Mdu0Builder` module responsible for initializing, modifying, and serializing the 8MB MDU #0 buffer.

**TDD Plan:** Create `nil_s3/pkg/builder/builder_test.go` first.

1.  **`TestInitEmptyMdu0`**:
    *   Create a new `Mdu0Builder` instance.
...
2.  **`TestAppendFileRecord`**:
    ...
    *   Add a `FileRecordV1` for "file1.txt" (1KB at offset 0).
...
**Definition of Done:** `go test ./pkg/builder` passes, demonstrating correct management of the 8MB MDU #0 buffer including File Table and Root Table operations.

### 9.3 Task 3: Refactor `GatewayUpload` (Service Layer)

**Description:** Modify the `GatewayUpload` handler (`nil_s3/main.go`) to utilize the `Mdu0Builder` and the new "Filesystem on Slab" logic.

**TDD Plan:** Create `nil_s3/main_test.go` (or a dedicated integration test package for handlers).

1.  **`TestUploadNewDealLifecycle`**:
    *   Mock `nil_cli` `shardFile` call to return dummy MDUs and roots.
    *   Call `GatewayUpload` for "file_A.txt" (no `deal_id` provided).
    *   Assert that the response includes a *newly generated* `deal_id` (e.g., by checking mock chain response) and the initial `manifest_root` for MDU #0 (containing just the FileTableHeader and Root for MDU #1).
    *   Simulate subsequent `GatewayUpdateDealContent` call to commit this.
2.  **`TestUploadAppendToExistingDeal`**:
    *   Pre-populate deal state: an existing `deal_id` on chain, and an MDU #0 file on disk.
    *   Call `GatewayUpload` for "file_B.txt", providing the `deal_id`.
    *   Assert that the response contains the `manifest_root` reflecting "file_B.txt" appended to the File Table, and the new `allocated_length`.
    *   Verify that `MDU #0`'s on-disk representation for this deal is updated correctly.
3.  **`TestUploadTombstoneReuse`**:
    ...
    *   Assert `MDU #0` is updated reflecting reuse (and splitting if necessary).
4.  **`TestUploadLargeFileExpansion`**:
    *   Upload a file spanning multiple MDUs.
    *   Assert that `Mdu0Builder` correctly adds multiple roots to the Root Table and `allocated_length` is updated correctly.

**Definition of Done:** The `GatewayUpload` handler correctly processes new file uploads, managing the MDU #0 state locally and returning the necessary information for `MsgUpdateDealContent` (new `ManifestRoot`, `allocated_length`).

### 9.4 Task 4: Refactor `GatewayFetch` (Filesystem Resolution)

**Description:** Update the `GatewayFetch` handler to resolve files by their path within a Deal, rather than requiring a direct CID for a single file.

**TDD Plan:** Extend `nil_s3/main_test.go` (or a dedicated integration test package for handlers).

1.  **`TestFetchByExistingPath`**:
    *   Setup deal state with "video.mp4" via `Mdu0Builder`.
    *   Call `GatewayFetch(deal_id, path="video.mp4")`.
    *   Assert that the correct `FileRecord` is located in MDU #0.
    *   Mock retrieval of the actual data MDUs. Assert the correct bytes are streamed back (e.g., via `httptest.ResponseRecorder`).
2.  **`TestFetchPathNotFound`**:
    *   Call `GatewayFetch` for a non-existent path within an active deal.
...
4.  **`TestFetchOffsetMapping`**:
    ...
    *   Assert that the internal logic correctly maps `Logical_Offset` to `Physical_MDU_Index` and `Offset_within_MDU`.

**Definition of Done:** `GatewayFetch` correctly resolves file paths within MDU #0, retrieves the corresponding `FileRecord`, and streams the correct byte ranges from the underlying Data MDUs.

### 9.5 Task 5: Refactor `GatewayUpdateDealContent` (Chain Interaction)
...
1.  **`TestFullDealLifecycle_E2E`**:
    ...
    *   **Phase 2: Create Deal:** Use `GatewayCreateDealFromEvm` to establish a base deal with `allocated_length = 1`.
    ...
    *   **Phase 6: Deletion & Reupload:** Simulate deletion, then re-upload a file to reuse space. Assert `manifest_root` updates correctly and `allocated_length` remains stable (if no new MDUs were needed).
