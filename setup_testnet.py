...
genesis_path = "polystorechain_data/config/genesis.json"
gentx_pattern = "polystorechain_data/config/gentx/*.json"
validator_key_path = "validator_key.json"

def get_gentx_file():
    gentx_files = glob.glob(gentx_pattern)
    if not gentx_files:
        print("Error: No gentx files found. Run `polystorechaind genesis gentx` first.")
        sys.exit(1)
...
if __name__ == "__main__":
    # Clean and setup
    os.system(f"rm -rf polystorechain_data")
    os.system(f"polystorechaind init testnode --chain-id testchain-1 --home polystorechain_data")
    os.system(f"polystorechaind keys add validator --home polystorechain_data --keyring-backend test --output json > {validator_key_path}")
    ...
    os.system(f"polystorechaind genesis add-genesis-account {validator_address_bech32} 1000000000000stake --home polystorechain_data --keyring-backend test")
    
    # Generate gentx
    os.system(f"polystorechaind genesis gentx validator 100000000stake --chain-id testchain-1 --home polystorechain_data --keyring-backend test")
    ...
    app_toml_path = os.path.join("polystorechain_data", "config", "app.toml")
    ...
    print("\nAttempting to start polystorechaind...")
    os.system(f"polystorechaind start --home polystorechain_data --pruning=nothing --minimum-gas-prices=0.0001stake > polystorechain.log 2>&1 &")
    os.system("sleep 10")
