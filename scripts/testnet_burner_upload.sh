...
Options:
  ...
  --keystore-password-env <name>  Env var containing keystore password (default: POLYSTORE_BURNER_KEYSTORE_PASSWORD)
  ...
  --faucet-auth-env <name>        Env var for faucet auth token (default: POLYSTORE_FAUCET_AUTH_TOKEN)
...
Notes:
  ...
  - Set POLYSTORE_BURNER_KEYSTORE_PASSWORD (or pass --keystore-password-env) to avoid an interactive keystore-password prompt.
...
FAUCET_AUTH_ENV="POLYSTORE_FAUCET_AUTH_TOKEN"
...
if [[ -z "${POLYSTORE_FAUCET_AUTH_TOKEN:-}" && -n "${POLYSTORE_TESTNET_FAUCET_AUTH_TOKEN:-}" ]]; then
  export POLYSTORE_FAUCET_AUTH_TOKEN="$POLYSTORE_TESTNET_FAUCET_AUTH_TOKEN"
fi
...
export POLYSTORE_NODE="${POLYSTORE_NODE:-${POLYSTORE_TESTNET_NODE:-https://rpc.polynomialstore.com}}"
export LCD_BASE
export POLYSTORE_GAS_PRICES="${POLYSTORE_GAS_PRICES:-${POLYSTORE_TESTNET_GAS_PRICES:-0.001aatom}}"
...
cleanup() {
  if [[ "${DESTROY_LOCAL_KEY_AFTER_EXPORT:-1}" == "1" ]]; then
    unset PRIVATE_KEY EVM_ADDRESS POLYSTORE_ADDRESS KEYSTORE_PASSWORD || true
  fi
}
trap cleanup EXIT
...
echo "NIL address: $POLYSTORE_ADDRESS"

if [[ "$SKIP_FAUCET" != "1" ]]; then
  faucet_auth_token="${!FAUCET_AUTH_ENV:-}"
  faucet_payload="$(printf '{"address":"%s"}' "$POLYSTORE_ADDRESS")"
  ...
  while true; do
    ...
    if balances_json="$(curl -fsS "$LCD_BASE/cosmos/bank/v1beta1/balances/$POLYSTORE_ADDRESS" 2>/dev/null)"; then
  ...
  if [[ "$funded" != "1" ]]; then
    echo "error: timed out waiting for funded balance on $POLYSTORE_ADDRESS" >&2
    exit 1
  fi
fi
