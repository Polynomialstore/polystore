...
current_epoch() {
  ...
  epoch_len=$($NILCHAIND query polystorechain params --home "$CHAIN_HOME" --output json | jq -r '.params.epoch_len_blocks // "0"')
...
CREATE_OUT=$($NILCHAIND tx polystorechain create-deal 1000 1000000 1000000 --service-hint "General:rs=2+1" --chain-id 31337 --from provider1 --yes --keyring-backend test --home "$CHAIN_HOME" --gas-prices 0.001aatom --output json)
