...
SERVICE_NAME="nilchaind"
ENV_FILE="/etc/nilstore/nilchaind.env"
...
Build and redeploy nilchaind for a systemd-managed hub node.

By default this script:
1) builds nilchaind from SOURCE_ROOT/nilchain,
...
Options:
  ...
  --service <name>          systemd service name (default: nilchaind)
  --env-file <path>         EnvironmentFile path (default: /etc/nilstore/nilchaind.env)
...
load_env_file() {
  ...
  NILCHAIND_BIN="${NILCHAIND_BIN:-$TARGET_ROOT/nilchain/nilchaind}"
  NIL_HOME="${NIL_HOME:-/var/lib/nilstore/nilchaind}"
}
...
preflight_build() {
  ...
  [ -d "$SOURCE_ROOT/nilchain" ] || die "nilchain dir not found under source root: $SOURCE_ROOT"
  [ -f "$SOURCE_ROOT/nilchain/go.mod" ] || die "nilchain/go.mod missing under source root: $SOURCE_ROOT"
...
build_nilchaind() {
  ...
  log "building nilchaind from $SOURCE_ROOT/nilchain"
  if [ "$DRY_RUN" -eq 1 ]; then
    printf '+ (cd %q && GOFLAGS=%q go build -o %q ./cmd/nilchaind)\n' \
      "$SOURCE_ROOT/nilchain" "$build_goflags" "$SOURCE_ROOT/nilchain/nilchaind"
    return 0
  fi

  (
    cd "$SOURCE_ROOT/nilchain"
    GOFLAGS="$build_goflags" go build -o "$SOURCE_ROOT/nilchain/nilchaind" ./cmd/nilchaind
  )
}

install_nilchaind() {
  local source_bin target_bin target_dir timestamp
  source_bin="$SOURCE_ROOT/nilchain/nilchaind"
...
verify_chain() {
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    ...
    if [ -n "$OPERATOR_ADDRESS" ]; then
      print_cmd curl -fsS --max-time 8 "$LCD_BASE/nilchain/nilchain/v1/provider-pairings/pending-by-operator/$OPERATOR_ADDRESS"
    fi
    return 0
  fi
  ...
  if [ -n "$OPERATOR_ADDRESS" ]; then
    ...
    pending_endpoint="$LCD_BASE/nilchain/nilchain/v1/provider-pairings/pending-by-operator/$OPERATOR_ADDRESS"
