...
require_cmd curl
require_cmd python3
[ -z "${POLYSTORE_BENCH_PROOFS_DIR:-}" ] || fail "POLYSTORE_BENCH_PROOFS_DIR is unsupported; fixtures must be generated by this run"
[ -z "${POLYSTORE_BENCH_MANIFEST_ROOT:-}" ] || fail "POLYSTORE_BENCH_MANIFEST_ROOT is unsupported; fixtures must be generated by this run"
...
GAS_LIMIT="${POLYSTORE_BENCH_GAS:-$(python3 -c 'import json,sys; print(json.load(sys.stdin)["gas_limit"])' <<<"$PROFILE_JSON")}"
...
prepare_proof_fixtures() {
  ...
  PROOF_MANIFEST_ROOT="$(python3 -c 'import json,sys; print(json.load(sys.stdin)["manifest_root"])' <<<"$FIXTURE_JSON")"
}
...
send_tx() { # send_tx <kind> <index> <from> <module-cli-args...>
  ...
  # Cobra prints usage before the actual error; preserve the bounded tail.
  [ "${#error}" -le 8192 ] || error="${error: -8192}"
  record_tx "$kind" "$index" "$from" "$hash" "$code" "$(( (finished - started) / 1000000 ))" "$before" "$after" "$error"
  if [ "$LAST_TX_OUTCOME" = unknown ]; then
    printf '%s\n' "$error" >&2
    fail "unknown transaction outcome; aborting before reusing its signer"
  fi
...
for i in $(seq 1 "$SESSIONS"); do
  ...
  if send_tx open-session "$i" "$USER_NAME" open-retrieval-session --deal-id "$DEAL_ID" --provider "$PROVIDER_ADDR" \
    ...
    if [ "$PROOFS_PER_SESSION" -eq 0 ]; then
      ...
      # Preserve the generated fixture hash; inject this run's session ID into
